...
def get_train_loader(args, dataset, height, width, batch_size, workers=0,
                    iters=None):
    ...
    train_set = sorted(dataset.images)

    train_loader = IterLoader(
            DataLoader(
                Preprocessor_tran(
                    train_set, root=dataset.root_dir, main_transform=train_transformer,
                        img_transform=img_transformer, gt_transform=gt_transformeer
                ),
                batch_size=batch_size, num_workers=0, sampler=None,
...
def get_test_loader(dataset, batch_size, workers = 0):
    normalizer = T.standard_transforms.Normalize(mean=[0.485, 0.456, 0.406],
                             std=[0.229, 0.224, 0.225])
    ...
    test_loader = DataLoader(
        Preprocessor(
            dataset=testset.train, root=dataset.root,
...
def main_worker(args):
    ...
    #Create Model定义模型和优化器
    model, optim = create_model(args)

    #Prepare data
    print("==> Load datasets")
    #加载训练集和测试集
    dataset_src = get_data(args.data_dir, source=True, num_domains=args.num_clustering)
    dataset_trg = get_data(args.data_dir, source=False)
    #加载测试集图像
    test_loader = get_test_loader(dataset_trg, args.test_batch_size, args.workers)
    train_loaders = get_train_loader(args,dataset=dataset_src,height=320,width=320,batch_size=4)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # start training
        trainer.train(
            epoch=epoch, data_loaders=train_loaders, optimizer=optimizer,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str,default = r'D:\conda3\Transfer_Learning\CrowdDataset\shanghai\ShanghaiTech\part_A')
    parser.add_argument('--logs-dir', type=str, default=osp.join('PATH', 'logs'))
