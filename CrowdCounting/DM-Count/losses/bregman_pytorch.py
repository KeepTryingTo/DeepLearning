...
def sinkhorn_knopp(a, b, C,
    ...
    device = a.device
    #TODO 对应目标分布和源分布的大小，也就是当前的代价矩阵的行和列
    na, nb = C.shape
    ...
    #TODO 如果自己没有初始化源分布u和目标分布v的话，就采用归一化的方式进行初始化
    ...
    while (err > stopThr and it <= maxIter):
        ...
        #TODO 判断矩阵u和矩阵v中是否存在nan值，如果存在的话就将和代价矩阵计算之前的值重新赋一遍
        ...
        if log and it % eval_freq == 0:
            ...
            # TODO b_hat = torch.sum(u.reshape(-1, 1) * K * v.reshape(1, -1), 0)
            # TODO 为了节约内存，使用下面这种方式 but with more memory efficient
            b_hat = torch.matmul(u, K) * v #TODO 计算当前的分配矩阵
            #TODO 当前计算的分配矩阵和一个边际约束的误差
    ...
    # TODO 将其源分布和目标分布展开成一样的维度transport plan，也就是得到分配矩阵P
