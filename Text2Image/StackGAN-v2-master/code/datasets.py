...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train', embedding_type='cnn-rnn',
        ...
        self.imsize = []
        #TODO 由于使用的多尺度，生成多个不同尺度的图像和不同尺度的判别器
        for i in range(cfg.TREE.BRANCH_NUM):
            self.imsize.append(base_size)
            base_size = base_size * 2
    ...
    def load_all_captions(self):
        ...
        #TODO 针对图像的对应上下文描述的其对应的embedding
        caption_dict = {}
    ...
    def load_embedding(self, data_dir, embedding_type):
        ...
        print('data dir: {}'.format(os.path.join(data_dir,embedding_filename)))
    ...
    def load_class_id(self, data_dir, total_num):
        if os.path.isfile(os.path.join(data_dir , 'class_info.pickle')):
            with open(os.path.join(data_dir , 'class_info.pickle'), 'rb') as f:
                if sys.version_info.major > 2:
    ...
    def prepair_training_pairs(self, index):
        ...
        if self.bbox is not None:
            ...
            bbox = None
            data_dir = self.data_dir
        # TODO captions = self.captions[key]（注意每张图像对应10句相同含义的不同描述）
        embeddings = self.embeddings[index, :, :]
        img_name = '%s/images/%s.jpg' % (data_dir, key)
        #TODO read image
        imgs = get_imgs(img_name, self.imsize,bbox, self.transform, normalize=self.norm)
        #TODO 随机的选择一张图像作为mismatch image
        wrong_ix = random.randint(0, len(self.filenames) - 1)
        #TODO 判断随机选择的image 和 当前的image是否是相同的类别
        """
        whereas negative sample pairs consist of two groups. The first is real images with mismatched text
        embeddings, while the second is synthetic images with their corresponding text embeddings.
        """
