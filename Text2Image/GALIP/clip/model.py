...
class VisionTransformer(nn.Module):
    def __init__(
            self, input_resolution: int, patch_size: int,
...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        if self.text_projection is not None:
            nn.init.normal_(self.text_projection, mean=0.0,std=self.transformer.width ** -0.5)
    ...
    def encode_text(self, text):
        ...
        #从eot嵌入中获取特征(eot_token是每个序列中的最大值)
        x = x[
    ...
    def forward(self, image, text):
        ...
        # normalized features 图像特征和文本特征的归一化处理
