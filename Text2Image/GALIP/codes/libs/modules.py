...
from codes.libs.utils import transf_to_CLIP_input, dummy_context_mgr
from codes.libs.utils import mkdir_p, get_rank
from codes.libs.datasets import prepare_data
...
############   GAN   ############
def train(dataloader, netG, netD, netC, text_encoder,
          image_encoder, optimizerG, optimizerD,
          scaler_G, scaler_D, args):
    batch_size = args.batch_size
    device = args.device
    epoch = args.current_epoch
    max_epoch = args.max_epoch
    z_dim = args.z_dim
    netG, netD, netC, image_encoder = (netG.train(), netD.train(),
                                       netC.train(), image_encoder.train())
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            # prepare_data
            real, captions, CLIP_tokens, sent_emb, words_embs, keys = prepare_data(data,
                                                                                   text_encoder, device)
            ...
            #TODO 将句子向量交错拼接在一起，得到一个失配的句子向量，对应假样本 predict mismatch
            mis_sent_emb = torch.cat(
                (sent_emb[1:],
                sent_emb[0:1]), dim=0
            ).detach()
            _, errD_mis = predict_loss(netC, real_feats, mis_sent_emb, negtive=True)
            # TODO synthesize fake images 生成噪声向量
            noise = torch.randn(batch_size, z_dim).to(device)
            fake = netG(noise, sent_emb)
            #TODO 对生成的假图像进行编码
            CLIP_fake, fake_emb = image_encoder(fake)
            fake_feats = netD(CLIP_fake.detach())#TODO 判断假图像
...
def test(dataloader, text_encoder,
         netG, PTM, device,
         m1, s1, epoch, max_epoch,
         times, z_dim, batch_size):
    FID, TI_sim = calculate_FID_CLIP_sim(dataloader, text_encoder,
                                         netG, PTM, device, m1, s1,
                                         epoch, max_epoch, times,
                                         z_dim, batch_size)
    return FID, TI_sim
...
def calculate_FID_CLIP_sim(dataloader, text_encoder,
                           netG, CLIP, device, m1, s1,
                           epoch, max_epoch, times,
                           z_dim, batch_size):
    """ Calculates the FID """
    ...
    for time in range(times):
        for i, data in enumerate(dataloader):
            ...
            # (1) Prepare_data
            ######################################################
            imgs, captions, CLIP_tokens, sent_emb, words_embs, keys = prepare_data(data,
                                                                                   text_encoder,
                                                                                   device)
            ######################################################
            # (2) Generate fake images
            ...
            with torch.no_grad():
                ...
                fake_imgs = torch.clamp(fake_imgs, -1., 1.)
                #TODO torch.nan_to_num 函数的作用是将输入张量中的 NaN（Not a Number）值、
                # 正无穷和负无穷替换为指定的数值。
                ...
                #TODO 图像归一化
                fake = norm(fake_imgs)
                #TODO 输入到inceptionv2中得到生成图像的预测结果
                pred = model(fake)[0]
                if pred.shape[2] != 1 or pred.shape[3] != 1:
                    pred = adaptive_avg_pool2d(pred, output_size=(1, 1))
                # TODO concat pred from multi GPUs
...
def sample_one_batch(noise, sent, netG, multi_gpus, epoch, img_save_dir, writer):
    if (multi_gpus==True) and (get_rank() != 0):
        ...
        with torch.no_grad():
            B = noise.size(0)
            #TODO 针对训练集以及测试集句子向量生成对应的假图像，最后拼接保存
            fixed_results_train = generate_samples(noise[:B//2],
                                                   sent[:B//2],
                                                   netG).cpu()
            torch.cuda.empty_cache()
            fixed_results_test = generate_samples(noise[B//2:],
                                                  sent[B//2:],
                                                  netG).cpu()
            torch.cuda.empty_cache()
            fixed_results = torch.cat((fixed_results_train,
                                       fixed_results_test), dim=0)
