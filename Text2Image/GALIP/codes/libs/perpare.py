...
import clip
import importlib
from codes.libs.utils import choose_model
...
def prepare_models(args):
    ...
    CLIP_txt_enc.eval()
    # GAN models
    netG = NetG(args.nf, args.z_dim, args.cond_dim,
                args.imsize, args.ch_size,
                args.mixed_precision, CLIP4trn).to(device)
    netD = NetD(args.nf, args.imsize, args.ch_size,
                args.mixed_precision).to(device)
...
def prepare_dataset(args, split, transform):
    ...
    from codes.libs.datasets import TextImgDataset as Dataset
