...
from codes.libs.utils import dummy_context_mgr
...
class CLIP_IMG_ENCODER(nn.Module):
    def __init__(self, CLIP):
        ...
        model = CLIP.visual
        # TODO 这里冻结住，不进行参数的更新print(model)
    ...
    def define_module(self, model):
        ...
        #TODO nn.Parameter(scale * torch.randn(width, output_dim))
        self.class_embedding = model.class_embedding
        self.positional_embedding = model.positional_embedding
        self.ln_pre = model.ln_pre #TODO 层归一化
        self.transformer = model.transformer
        self.ln_post = model.ln_post#TODO 层归一化
        #TODO nn.Parameter(scale * torch.randn(width, output_dim))
    ...
    def forward(self, img: torch.Tensor):
        ...
        x = x.permute(1, 0, 2)
        # Local features
        #TODO 这里从CLIP-VIT中选择指定层进行后面的信息注入 selected = [1,4,7,12]
        selected = [1,4,8]
        local_features = []
        for i in range(12):
            ...
            if i in selected:
                local_features.append(
                    x.permute(1, 0, 2)[:, 1:, :].permute(0, 2, 1).reshape(-1, 768, grid, grid).contiguous().type(img.dtype))
...
class CLIP_TXT_ENCODER(nn.Module):
    ...
    def define_module(self, CLIP):
        ...
        self.ln_final = CLIP.ln_final #TODO 层归一化操作
        #TODO text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim))
    ...
    def forward(self, text):
        ...
        sent_emb = x[torch.arange(x.shape[0]),
...
class NetD(nn.Module):
    def __init__(self, ndf, imsize, ch_size, mixed_precision):
        ...
        self.main = D_Block(768, 512, 3, 1, 1,
                            res=True, CLIP_feat=False)
