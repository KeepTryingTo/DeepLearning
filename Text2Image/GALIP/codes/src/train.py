...
from codes.libs.utils import (mkdir_p,get_rank,merge_args_yaml,
                              get_time_stamp,save_args)
from codes.libs.utils import (load_models_opt,save_models_opt,save_models,
                              load_npz,params_count)
from codes.libs.perpare import prepare_dataloaders,prepare_models
from codes.libs.modules import sample_one_batch as sample, test as test, train as train
from codes.libs.datasets import get_fix_data
...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        type=str, default='../cfg/birds.yml',
    ...
    parser.add_argument('--model', type=str,
                        default='GALIP',
                        help='the model for training')
    parser.add_argument('--state_epoch', type=int,
                        default=1,
                        help='state epoch')
    ...
    parser.add_argument('--multi_gpus', type=str,
                        default='False',
                        help='if use multi-gpu')
...
def main(args):
    ...
    stamp = '_'.join([str(args.model),'nf'+str(args.nf),
                      str(args.stamp),str(args.CONFIG_NAME),
                      str(args.imsize),time_stamp])
    #TODO 保存模型的路径
    args.model_save_file = osp.join(ROOT_PATH, 'saved_models',
                                    str(args.CONFIG_NAME), stamp)
    ...
    # TODO prepare TensorBoard
    if (args.multi_gpus==True) and (get_rank() != 0):
        writer = None
    else:
        writer = SummaryWriter(log_dir)
    # TODO prepare dataloader, models, data
    ...
    fixed_img, fixed_sent, fixed_words, fixed_z = get_fix_data(train_dl, valid_dl,
                                                               text_encoder, args)
    #TODO 是否采用多GPU
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    ...
    start_epoch = 1
    #TODO 加载模型 load from checkpoint
    if args.state_epoch!=1:
        ...
        netG, netD, netC, optimizerG, optimizerD = load_models_opt(netG,
                                                                   netD,
                                                                   netC,
                                                                   optimizerG,
                                                                   optimizerD,
    ...
    FID, TI_score = test(valid_dl, text_encoder,
                         netG, CLIP4evl, args.device,
                         m1, s1, 0, args.max_epoch,
                         args.sample_times, args.z_dim,
                         args.batch_size)
    print('FID: {}  TI_score: {}'.format(FID,TI_score))
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        torch.cuda.empty_cache()
        train(train_dl, netG, netD, netC,
              text_encoder, image_encoder,
              optimizerG, optimizerD, scaler_G,
              scaler_D, args)
        torch.cuda.empty_cache()
        # save
        if epoch%save_interval==0:
            save_models_opt(netG, netD, netC,
                            optimizerG, optimizerD,
                            epoch, args.multi_gpus,
                            args.model_save_file)
            torch.cuda.empty_cache()
        # sample
        if epoch%gen_interval==0:
            sample(fixed_z, fixed_sent, netG,
                   args.multi_gpus, epoch,
