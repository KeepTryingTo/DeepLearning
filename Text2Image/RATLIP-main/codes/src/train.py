...
from codes.lib.utils import load_models_opt,save_models_opt,save_models,load_npz,params_count
...
def main(args):
    ...
    # prepare dataloader, models, data TODO 加载数据集
    train_dl, valid_dl ,train_ds, valid_ds, sampler = prepare_dataloaders(args)
    #TODO 加载模型CLIP，图像编码器，文本编码器以及生成器和判别器
    ...
    start_epoch = 1
    # load from checkpoint TODO 加载之前中间保存的权重文件
    ...
    # print args TODO 如果不采用多GPU方式，则将当前的所有配置信息保存到args.yaml文件中
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
        pprint.pprint(args)
    ...
    # Start training TODO 测试间隔，生成图像间隔，保存模型间隔
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        if epoch%gen_interval==0:
            #TODO 生成的随机噪声和句子向量
            sample(noise=fixed_z, sent=fixed_sent,
                   netG=netG, multi_gpus=args.multi_gpus, epoch=epoch,
                   img_save_dir=args.img_save_dir, writer=writer)
            torch.cuda.empty_cache()
        # test TODO 测试阶段，计算FID和TI_score分数
