...
from codes.lib.utils import dummy_context_mgr
from codes.models.ShuffleAttention import *
...
class CLIP_IMG_ENCODER(nn.Module):
    ...
    def forward(self, img: torch.Tensor):
        ...
        x = self.ln_post(x[:, 0, :]) #TODO layernorm
...
class CLIP_TXT_ENCODER(nn.Module):
    ...
    def forward(self, text):
        ...
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype) # TODO layernorm
...
class Affine_rnn(nn.Module):
    ...
    def forward(self, x, y=None):
        if y is not None:
            ...
            _, (h, c) = self.rnn(
                torch.zeros(
                    1, x.size(0), self.rnn.hidden_size
                ).to(x.device)
            )
