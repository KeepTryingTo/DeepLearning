...
from codes.lib.utils import mkdir_p,get_rank,merge_args_yaml,get_time_stamp,save_args
from codes.lib.utils import load_model_opt,save_models,load_npz
from codes.lib.perpare import prepare_dataloaders,prepare_models
from codes.lib.modules import sample_one_batch as sample, test as test, train as train
from codes.lib.datasets import get_fix_data
...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file', type=str,
                        default='../cfg/bird.yml',
                        help='optional config file')
    parser.add_argument('--num_workers', type=int, default=8,help='number of workers(default: 4)')
...
def main(args):
    ...
    # TODO 加载数据集prepare dataloader, models, data
    ...
    if args.resume_epoch!=1:
        ...
        netG, netD, netC, optimizerG, optimizerD = load_model_opt(netG, netD, netC,
                                                                  optimizerG,
                                                                  optimizerD, path,
                                                                  args.multi_gpus)
    # print args
    ...
    for epoch in range(strat_epoch, args.max_epoch, 1):
        ...
        if epoch%test_interval==0:
            torch.cuda.empty_cache()
            fid = test(valid_dl, text_encoder, netG,
                       args.device, m1, s1, epoch,
                       args.max_epoch, \
                        args.sample_times, args.z_dim,
                       args.batch_size, args.truncation,
                       args.trunc_rate)
        if (args.multi_gpus==True) and (get_rank() != 0):
            None
        else:
