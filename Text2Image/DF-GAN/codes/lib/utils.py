...
def truncated_noise(batch_size=1, dim_z=100, truncation=1., seed=None):
    ...
    """
    生成截断正态分布随机数:
    truncnorm.rvs(-2, 2, ...)：从截断正态分布中随机生成样本。这里选择的截断区间是 
        -2 到 2，表示生成的样本会被限制在这个范围内。
    size=(batch_size, dim_z)：指定生成的随机数数组的形状，
        其中 batch_size 是每次生成的样本数量，dim_z 是每个样本的维度（即每个随机向量的长度）。
    random_state=state：确保生成的随机数序列是可复现的（如果使用了种子）。
    """
    values = truncnorm.rvs(-2, 2,
                           size=(batch_size, dim_z),
                           random_state=state).astype(np.float32)
    """
    将生成的随机样本 values 乘以一个 truncation 值，通常这个值在 [0, 1] 范围内，
    用于控制生成样本的“显著性”或“强度”。较小的 truncation 值会使得生成的样本更加集中，朝着截断边界靠近。
    最终返回经过截断放大后处理后的随机样本。
    """
...
def load_models(netG, netD, netC, path):
    ...
    netD = load_model_weights(netD, checkpoint['model']['netD'],multi_gpus=False)
    netC = load_model_weights(netC, checkpoint['model']['netC'],multi_gpus=False)
