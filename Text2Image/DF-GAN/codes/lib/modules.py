...
from codes.lib.utils import truncated_noise
from codes.lib.utils import mkdir_p, get_rank

from codes.lib.datasets import prepare_data, encode_tokens
...
def train(dataloader, netG, netD, netC, text_encoder, optimizerG, optimizerD, args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        # TODO sim = MAP(image_encoder, fake, sent_emb).mean()
...
def MA_GP(img, sent, out):
    """TODO
        ...
        out: for real image discriminator's output
        grad_outputs=torch.ones(out.size()).cuda(): 指定对 out 的输出梯度为全1，
                        表明我们需要关于 out 的直接梯度。
    """
    grads = torch.autograd.grad(outputs=out,
                            ...
                            only_inputs=True)
    """
    grads 是一个包含 img 和 sent 的梯度的元组。
    grad0 和 grad1 分别是 img 和 sent 的梯度，这里将它们展平为二维张量，其中第一维是样本数，第二维是特征数。
    """
