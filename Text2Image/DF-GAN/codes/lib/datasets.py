...
def get_fix_data(train_dl, test_dl, text_encoder, args):
    #TODO 获得训练集的图像，词向量以及句子向量
    fixed_image_train, fixed_word_train, fixed_sent_train = get_one_batch_data(train_dl,
                                                                               text_encoder,
                                                                               args)
    # TODO 获得测试集的图像，词向量以及句子向量
    fixed_image_test, fixed_word_test, fixed_sent_test = get_one_batch_data(test_dl,
                                                                            text_encoder,
                                                                            args)
...
def sort_sents(captions, caption_lens):
    # TODO 根据句子的长度进行降序排序 sort data by the length in a decreasing order
...
class TextImgDataset(data.Dataset):
        ...
        for i in range(len(filenames)):
            ...
            with open(cap_path, "r") as f:
                ...
                for cap in captions:
                    ...
                    # and drops everything else TODO 对句子进行分词
    ...
    def get_caption(self, sent_ix):
        #TODO  a list of indices for a sentence
        sent_caption = np.asarray(self.captions[sent_ix]).astype('int64')
        if (sent_caption == 0).sum() > 0:
            print('ERROR: do not need END (0) token', sent_caption)
        #TODO 句子的长度
        num_words = len(sent_caption)
        # TODO pad with 0s (i.e., '<end>')
        x = np.zeros((self.word_num, 1), dtype='int64')
        x_len = num_words
        #TODO 判断句子的长度和指定词的数量大小，也就是太长的句子需要省略掉后面的
        if num_words <= self.word_num:
            x[:num_words, 0] = sent_caption
        else:
            #TODO 如果大于指定长度就将词的索引随机打散
            ix = list(np.arange(num_words))  # 1, 2, 3,..., maxNum
            np.random.shuffle(ix)
            #TODO 然后获得打散之后的指定长度句子词
            ix = ix[:self.word_num]
            #TODO 按照索引又重新排序
            ix = np.sort(ix)
            #TODO 获得对应的词
    ...
    def __getitem__(self, index):
        ...
        # TODO 随机选择一个句子 random select a sentence
