    ...
    def __init__(self, ndf, nef, bcondition=True):
        ...
        if bcondition:
            self.outlogits = nn.Sequential(
                ...
                nn.Conv2d(in_channels=ndf * 8, out_channels=1, kernel_size=4, stride=4),
                nn.Sigmoid()
            )
        else:
            self.outlogits = nn.Sequential(
                nn.Conv2d(ndf * 8, out_channels=1, kernel_size=4, stride=4),
                nn.Sigmoid()
            )
...
class STAGE1_G(nn.Module):
    ...
    def define_module(self):
        ...
        ngf = self.gf_dim
        # TODO 对文本编码向量进行采样获得新的分布 TEXT.DIMENSION -> GAN.CONDITION_DIM
        self.ca_net = CA_NET(self.device)
        ...
        self.img = nn.Sequential(
            conv3x3(ngf // 16, out_planes=3),
            nn.Tanh())

    def forward(self, text_embedding, noise):
        #TODO 得到条件编码（通过重参数化技巧得到），以及均值和方差
        c_code, mu, logvar = self.ca_net(text_embedding)
        z_c_code = torch.cat((noise, c_code), 1)
        h_code = self.fc(z_c_code)

        h_code = h_code.view(-1, self.gf_dim, 4, 4)
        #TODO 生成器上采样最终得到64x64的低分辨率图像
...
class STAGE2_G(nn.Module):
    def __init__(self, STAGE1_G):
        ...
        # fix parameters of stageI GAN TODO 训练第二阶段的时候，将第一阶段冻结
    ...
    def forward(self, text_embedding, noise):
        #TODO 得到第一阶段生成的低分辨率图像
        _, stage1_img, _, _ = self.STAGE1_G(text_embedding, noise)
        stage1_img = stage1_img.detach()
        #TODO 第一阶段生成的低分辨率图像进行编码
        encoded_img = self.encoder(stage1_img)
        #TODO 通过文本向量得到采样之后的分布以及均值和方差
        c_code, mu, logvar = self.ca_net(text_embedding)
        c_code = c_code.view(-1, self.ef_dim, 1, 1)
