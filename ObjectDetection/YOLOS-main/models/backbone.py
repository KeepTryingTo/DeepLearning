...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        qkv = self.qkv(x).reshape(B, N, 3,
                                  self.num_heads,
                                  C // self.num_heads
                                  ).permute(2, 0, 3, 1, 4)  # 3, B, num_head, N, c
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16,
                 ...
                 norm_layer=nn.LayerNorm, is_distill=False):
        super().__init__()
        ...
        #TODO vision transform中用于最终结果的分类输出
    ...
    def finetune_det(self,
        ...
        #TODO 获得高宽划分patch之后的单元数量
        P_H, P_W = (self.img_size[0] // self.patch_size,
                    self.img_size[1] // self.patch_size)
        #TODO 改变patch embedding的形状
        ...
        patch_pos_embed = nn.functional.interpolate(patch_pos_embed,
                                                    ...
                                                    mode='bicubic',
                                                    align_corners=False)
        #TODO [B,E,P_H * P_W] => [B,P_H * P_W,E]
        patch_pos_embed = patch_pos_embed.flatten(2).transpose(1, 2)
        self.pos_embed = torch.nn.Parameter(torch.cat((cls_pos_embed,
                                                       patch_pos_embed,
                                                       det_pos_embed), dim=1))
        self.img_size = img_size
        if mid_pe_size == None:
    ...
    def InterpolateInitPosEmbed(self, pos_embed, img_size=(800, 1344)):
        ...
        #TODO 获得patch embedding
        patch_pos_embed = pos_embed[:, 1:-self.det_token_num, :]

        #TODO [B,P_H * P_W,E] => [B,E,P_H * P_W]
        patch_pos_embed = patch_pos_embed.transpose(1,2)
        B, E, Q = patch_pos_embed.shape
    ...
    def InterpolateMidPosEmbed(self, pos_embed, img_size=(800, 1344)):
        ...
        P_H, P_W = (self.mid_pe_size[0] // self.patch_size,
        ...
        scale_pos_embed = torch.cat((cls_pos_embed,
                                     patch_pos_embed,
                                     det_pos_embed), dim=2)
        return scale_pos_embed
    ...
    def forward_features(self, x):
        ...
        # TODO interpolate init pe 如果当前定义的patch embedding维度和1 + self.det_token_num
        #  不相同的话就进行插值操作
        #  其中patch embed = cat(cls_pos_embed, patch_pos_embed, det_pos_embed)
        if (self.pos_embed.shape[1] - 1 - self.det_token_num) != x.shape[1]:
            temp_pos_embed = self.InterpolateInitPosEmbed(self.pos_embed,
                                                          img_size=(H,W))
        else:
            temp_pos_embed = self.pos_embed
        # TODO interpolate mid pe
        if self.has_mid_pe:
            # temp_mid_pos_embed = []
            if (self.mid_pos_embed.shape[2] - 1 - self.det_token_num) != x.shape[1]:
                temp_mid_pos_embed = self.InterpolateMidPosEmbed(self.mid_pos_embed,
                                                                 img_size=(H,W))
            else:
                temp_mid_pos_embed = self.mid_pos_embed

        #TODO pos embedding + cls token + det token
...
def tiny(pretrained=None, **kwargs):
    ...
    if pretrained: 
        # checkpoint = torch.load('deit_tiny_patch16_224.pth', map_location="cpu")
