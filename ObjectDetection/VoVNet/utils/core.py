...
from data import (
                  VOCDetection, detection_collate, preproc)
...
def anchors(config):
    ...
    print('min size box: {}'.format(cfg['min_sizes']))
    print('max size box: {}'.format(cfg['max_sizes']))
...
def set_criterion(cfg,device):
    return MultiBoxLoss(num_classes=cfg.model.num_classes,
...
def get_dataloader(cfg, dataset, setname='train_sets'):
    ...
    Dataloader_function = {'VOC': VOCDetection}
...
def save_checkpoint(net, cfg, final=True, datasetname='COCO', epoch=10):
    ...
    if final:
        torch.save(net.state_dict(), weights_save_path +
                   'Final_vovnet_{}_size{}.pth'.format(datasetname,
                                                      cfg.model.input_size))
    else:
        torch.save(net.state_dict(), weights_save_path +
                   'vovnet_{}_size{}_epoch{}.pth'.format(datasetname,
                                                        cfg.model.input_size, epoch))
...
def nms_process(num_classes, i, scores, boxes, cfg,
                min_thresh, all_boxes, max_per_image):
