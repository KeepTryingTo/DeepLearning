...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, priors, targets):
        ...
        for idx in range(num):
            ...
            match(threshold=self.threshold, truths=truths,
                  priors=defaults, variances=self.variance,
                  labels=labels, landms=landms, loc_t=loc_t,
                  conf_t=conf_t, landm_t=landm_t, idx=idx)
        if GPU:
            loc_t = loc_t.cuda()
            conf_t = conf_t.cuda()
        ...
        # TODO Localization Loss (Smooth L1)
