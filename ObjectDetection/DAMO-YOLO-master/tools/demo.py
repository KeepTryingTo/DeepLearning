...
class Infer():
    def __init__(self, config,
                 infer_size=640,
                 device='cuda',
                 output_dir='/home/ff/myProject/KGT/myProjects/myProjects/DAMO-YOLO-master/outputs',
                 ckpt=None, end2end=False):
        ...
        if isinstance(infer_size,list) or isinstance(infer_size,tuple):
            self.infer_size = infer_size
        elif isinstance(infer_size,int):
            self.infer_size = (infer_size,infer_size)
    ...
    def _build_engine(self, config, engine_type):
        ...
        if engine_type == 'torch':
            ...
            model, self.input_name, self.infer_size, _, _ = self.build_onnx_engine(
                onnx_path=self.ckpt_path
            )
        else:
    ...
    def forward(self, origin_image):
        ...
        if self.engine_type == 'torch':
            ...
            output = self.model.run(output_names=None,
                                    input_feed={self.input_name: image_np})

        elif self.engine_type == 'tensorRT':
            image_np = np.asarray(image.tensors.cpu()).astype(np.float32)
            output = self.model(image_np)

        bboxes, scores, cls_inds = self.postprocess(output, image,
                                                    origin_shape=origin_shape)

        return bboxes, scores, cls_inds

    def visualize(self, image, bboxes, scores,
                  cls_inds, conf,
                  save_name='vis.jpg', save_result=True):
...
def make_parser():
    ...
    parser.add_argument('-p',
                        '--path',
                        default='/home/ff/myProject/KGT/myProjects/myProjects/DAMO-YOLO-master/images',
                        type=str,
    ...
    parser.add_argument('--output_dir',
                        default='/home/ff/myProject/KGT/myProjects/myProjects/DAMO-YOLO-master/outputs',
                        type=str,
