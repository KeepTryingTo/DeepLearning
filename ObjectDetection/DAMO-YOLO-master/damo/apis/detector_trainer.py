...
class Trainer:
    def __init__(self, cfg, args, tea_cfg=None, is_train=True):
        ...
        setup_logger(self.file_name,distributed_rank=get_rank(),mode='w',)

        # logger
        logger.info('args info: {}'.format(self.args))
        logger.info('cfg value:\n{}'.format(self.cfg))

        # TODO 加载模型 build model
        ...
        if tea_cfg is not None:
            ...
            self.feature_loss = FeatureLoss(
                channels_s=self.model.neck.out_channels,
                channels_t=self.tea_model.neck.out_channels,
                distiller='cwd'
            ).to(self.device)
        ...
        self.setup_iters(iters_per_epoch=iters,
                         start_epoch=self.start_epoch,
                         total_epochs=cfg.train.total_epochs,
                         warmup_epochs=cfg.train.warmup_epochs,
                         no_aug_epochs=cfg.train.no_aug_epochs,
                         eval_interval_epochs=cfg.miscs.eval_interval_epochs,
                         ckpt_interval_epochs=cfg.miscs.ckpt_interval_epochs,
                         print_interval_iters=cfg.miscs.print_interval_iters)

        self.lr_scheduler = cosine_scheduler(
            base_lr_per_img=cfg.train.base_lr_per_img,
            batch_size=cfg.train.batch_size,
            min_lr_ratio=cfg.train.min_lr_ratio,
            total_iters=self.total_iters,
            no_aug_iters=self.no_aug_iters,
            warmup_iters=self.warmup_iters,
            warmup_start_lr=cfg.train.warmup_start_lr
        )
    ...
    def get_data_loader(self, cfg):
        ...
        train_dataset = build_dataset(
            cfg=cfg,
            ann_files=cfg.dataset.train_ann,
            is_train=True,
            mosaic_mixup=cfg.train.augment.mosaic_mixup
        )
        val_dataset = build_dataset(cfg, cfg.dataset.val_ann, is_train=False)

        iters_per_epoch = math.ceil(len(train_dataset[0]) /cfg.train.batch_size)  # train_dataset is a list, however,

        train_loader = build_dataloader(datasets=train_dataset,
        ...
        val_loader = build_dataloader(datasets=val_dataset,
                                      augment=cfg.test.augment,
    ...
    def train(self, local_rank):
        ...
        # logger.info('Model Summary: {}'.format(get_model_info(self.model, (infer_shape, infer_shape))))
        ...
        for data_iter, (inps, targets, ids) in enumerate(self.train_loader):
            ...
            #TODO 如果当前的迭代次数大于指定不增强的迭代次数时就进行下后面数据增强操作
