...
class GiraffeNeckV2(nn.Module):
    def __init__(
        ...
        # TODO node x3: input x0, x1
        self.bu_conv13 = Conv(in_channels[1], in_channels[1], 3, 2, act=act)
        self.merge_3 = CSPStage(block_fn=block_name,
                                ch_in=in_channels[1] + in_channels[2],
                                ch_hidden_ratio=hidden_ratio,
        ...
        self.merge_6 = CSPStage(block_fn=block_name,
                                ch_in=in_channels[1] + out_channels[1] +in_channels[2],
                                ch_hidden_ratio=hidden_ratio,
                                ch_out=out_channels[2],
                                n = round(3 * depth),
                                act=act,
                                spp=spp,
                                depthwise=depthwise)
