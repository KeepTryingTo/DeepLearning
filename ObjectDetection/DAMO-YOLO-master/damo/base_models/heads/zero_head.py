...
class ZeroHead(nn.Module):
    ...
    def _init_layers(self):
        ...
        for i in range(len(self.strides)):
            cls_convs, reg_convs = self._build_not_shared_convs(
                ...
                feat_channels=self.feat_channels[i]
            )
    ...
    def forward_train(self, xin, labels=None, imgs=None, aux_targets=None):
        ...
        # TODO gt box和gt label => prepare labels during training
        b, c, h, w = xin[0].shape
        ...
        # TODO 根据下采样步长生成anchor box  prepare priors for label assignment and bbox decode
        mlvl_priors_list = [
            self.get_single_level_center_priors(batch_size=xin[i].shape[0],
                                                featmap_size=xin[i].shape[-2:],
                                                stride=stride,
                                                dtype=torch.float32,
                                                device=xin[0].device)
        ...
        # TODO forward for bboxes and classification prediction
    ...
    def forward_single(self, x, cls_convs, reg_convs, gfl_cls, gfl_reg, scale):
        ...
        if self.training:
            ...
            cls_score = cls_score.flatten(start_dim=2).permute(
                0, 2, 1)  # TODO N, h*w, self.num_classes+1
            bbox_pred = bbox_pred.flatten(start_dim=3).permute(
                0, 3, 1, 2)  # TODO N, h*w, 4, self.reg_max+1
    ...
    def get_single_level_center_priors(self,
        ...
        y = y.flatten()
        x = x.flatten()
        #TODO 使用 new_full 创建一个与 x 长度相同的张量，
        # 填充内容为 stride。这个张量将用于表示每个框的步长。
    ...
    def loss(
        ...
        # TODO 根据anchor对box进行解码操作 get decoded bboxes for label assignment
        #TODO 其中integral: => [b, hw, 4]
    ...
    def get_target_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            dfl_targets[pos_inds, :] = (bbox2distance(
                points=center_priors[pos_inds, :2] / center_priors[pos_inds, None, 2],
                bbox=pos_bbox_targets / center_priors[pos_inds, None, 2],
                max_dis=self.reg_max))
        if len(neg_inds) > 0:
