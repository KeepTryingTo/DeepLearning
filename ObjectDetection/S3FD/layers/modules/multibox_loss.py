...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num):
            ...
            self.match(threshold=self.threshold, truths=truths,
                       priors=defaults, variances=self.variance,
                       labels=labels,
                       loc_t=loc_t, conf_t=conf_t, idx=idx)
        ...
        #TODO 由于刚才在match函数中将IOU大于th1和小于th2位置confidence设置为1，
        # 这里在计算正样本数量的时候就因为刚才的操作而增加了正样本的数量
        pos = conf_t > 0
        num_pos = pos.sum(dim=1, keepdim=True)
        # TODO 计算定位损失Localization Loss (Smooth L1)
        #TODO  Shape: [batch,num_priors,4]
        ...
        # TODO Sum of losses: L(x,c,l,g) = (Lconf(x, c) + αLloc(x,l,g)) / N
