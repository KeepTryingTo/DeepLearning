...
def match(threshold, truths, priors,
    ...
    #TODO 计算真实框和anchor box之间的IOU
    overlaps = jaccard(
        truths,
        point_form(priors)
    )
    # TODO 以下和SSD中实现的方式差不多(Bipartite Matching)
    ...
    #TODO 将IOU大于th2（包含th3）和IOU大于th3的数量加起来除以2.并向下取整，
    # 也就是求解尺度“补偿”中anchor匹配数量的平均值
    N = torch.div(torch.sum(best_prior_overlap >= _th2) +
                  torch.sum(best_prior_overlap >= _th3),
                  other = 2,
                  rounding_mode='floor')
    matches = truths[best_truth_idx]          # Shape: [num_priors,4]
    conf = labels[best_truth_idx]         # Shape: [num_priors]
    conf[best_truth_overlap < _th2] = 0  #TODO  label as background

    best_truth_overlap_clone = best_truth_overlap.clone()
    #TODO 对于IOU大于th1和小于th2的位置值，
    add_idx = best_truth_overlap_clone.gt(_th1).eq(best_truth_overlap_clone.lt(_th2))
    #TODO 将大于th1和小于th2的位置之外的IOU值都设置为0
    best_truth_overlap_clone[~add_idx] = 0

    #TODO 对IOU进行降序排序，分别得到降序之后的值以及对应的索引
    stage2_overlap, stage2_idx = best_truth_overlap_clone.sort(descending=True)
    #TODO 由于小于th1和大于th2的位置都设置为0了，因此这里大于th1得到位置就是大于th1和小于th2的位置
    stage2_overlap = stage2_overlap.gt(_th1)

    if N > 0:
        #TODO 首先判断N和前N个值里面包含大于th1和小于th2的数量之间关系，
        # 如果小于N，则就是等于大于th1和小于th2的数量，否则上限设置为N
        N = torch.sum(stage2_overlap[:N]) if torch.sum(
            stage2_overlap[:N]) < N else N
        #TODO 将这些位置的置信度都+1，这里之所以+1，应该是为了增加正样本的数量，
        # 因为本身在众多的anchor box中人脸的数量非常少，因此使用这种方式增加正样本的数量，
        # 也就是通过尺度“补偿”匹配的那些标签设置为正样本
        conf[stage2_idx[:N]] += 1
