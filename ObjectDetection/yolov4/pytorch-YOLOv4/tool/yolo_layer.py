...
def yolo_forward_dynamic(output, conf_thresh, num_classes,
                         anchors, num_anchors, scale_x_y,
                         only_objectness=1,
                         validation=False):
    # Output would be invalid if it does not satisfy this assert
    # assert (output.size(1) == (5 + num_classes) * num_anchors)
    ...
    #TODO 根据当前层每一个grid cell指定的anchor box数量获得当前层的输出：中心坐标，高宽，置信度 以及类别概率
    for i in range(num_anchors):
        begin = i * (5 + num_classes)
        end = (i + 1) * (5 + num_classes)
...
class YoloLayer(nn.Module):
    ...
    '''
    def __init__(self,
                 anchor_mask=[], num_classes=0, anchors=[],
