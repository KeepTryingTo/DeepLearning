...
def train(model, device, config, epochs=5, batch_size=1,
    ...
    if True:
        ...
        evaluate(eval_model, val_loader, config, device)
...
def evaluate(model, data_loader, cfg, device, logger=None, **kwargs):
    ...
    for images, targets in data_loader:
        ...
        for step, (img, target, boxes, confs) in enumerate(zip(images, targets, outputs[0], outputs[1])):
...
def get_args(**kwargs):
    ...
    parser.add_argument('-pretrained', type=str,
                        default=r'./weights/yolov4.conv.137.pth',
