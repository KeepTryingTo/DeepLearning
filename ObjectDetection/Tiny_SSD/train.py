...
import shutil
import argparse
from models.tiny_ssd import TinySSD
...
from utils.core import (print_info,set_logger,set_optimizer,
                        set_criterion,init_net,anchors,
                        get_dataloader,save_checkpoint,
                        adjust_learning_rate,write_logger,
                        print_train_log)

parser = argparse.ArgumentParser(description='Tiny SSD Training')
parser.add_argument('-c', '--config', default='configs/tiny_ssd_COCO.py')
...
print_info('----------------------------------------------------------------------\n'
           '|                       TinySSD Training Program                       |\n'
           '----------------------------------------------------------------------', ['yellow', 'bold'])
#TODO 加载日志logger
logger = set_logger(args.tensorboard)
#TODO 加载配置文件
cfg = Config.fromfile(args.config)
net = TinySSD(num_classes=cfg.model.num_classes,
              anchor_nums=cfg.model.anchor_config.anchor_nums)
...
criterion = set_criterion(cfg,device)
...
if __name__ == '__main__':
    ...
    max_iter = cfg.train_cfg.step_lr[-1] * 1000
    ...
    for iteration in range(start_iter, max_iter):
        ...
        # print('priors.shape: {}'.format(priors.size()))
        ...
        print_train_log(iteration, cfg.train_cfg.print_epochs,
                        [time.ctime(), epoch, iteration % epoch_size, epoch_size,
                         iteration, loss_l.item(), loss_c.item(), load_t1 - load_t0, lr])
