...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, Backbone, Neck, Head, cfg):
        ...
        self.detect = Detect(self.num_classes , bkg_label=0,
                             top_k=200, conf_thresh=0.01,
                             nms_thresh=0.45,variance = cfg['variance'])
    ...
    def forward(self, x, phase):
        ...
        if phase == "test":
            #TODO https://blog.csdn.net/qq_36926037/article/details/108419899/
...
def build_ssd(phase, size=300, cfg=None):
    ...
    if size != 300 and size!=600 and size!=800:
        ...
        return
    print(phase)
    base = Backbone(cfg['model'],feature_map=[6,7,8,9,10,11])
    #neck=None
    neck = Neck(in_channels = cfg['backbone_out'], out_channels = cfg['neck_out'])
    head = SSDHead(num_classes = cfg['num_classes'],in_channels = cfg['neck_out'],
