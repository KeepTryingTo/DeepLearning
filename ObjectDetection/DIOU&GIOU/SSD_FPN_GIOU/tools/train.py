...
from tools.eval import test_net
...
parser.add_argument('--visdom', default=None,type=str,
                    help='Use visdom')
parser.add_argument('--work_dir',
                    default=r'/home/ff/myProject/KGT/myProjects/myProjects/awesome_SSD_FPN_GIoU-master/work_dir/',
...
def train():
    ...
    '''
    print('train dataset: {}'.format(args.dataset))
    ...
    #build, load,  the net 
    ssd_net = build_ssd(phase='train',
                        size = cfg['min_dim'],
                        cfg = cfg)
    ssd_net.priors.to(device)
    '''
    ...
    criterion = MultiBoxLoss(cfg = cfg,overlap_thresh = 0.5, 
                            ...
                            encode_target = False, device = device,
                             loss_c = train_config['loss_c'],
                             loss_r = train_config['loss_r'])
    #Vis
    if args.visdom:
        import visdom
    ...
    for epoch in range(max_epoch):
        ...
        if epoch % 10 == 0 and epoch > 60:#epoch>1000 and epoch % 50 == 0:
            ...
            torch.save(net.state_dict(),args.work_dir+cfg['work_name']+'/giou_ssd_fpn_'+
                       repr(epoch)+'.pth')
        if args.visdom: 
