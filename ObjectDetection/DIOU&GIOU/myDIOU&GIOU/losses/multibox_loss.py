...
from losses.box_utils import match, log_sum_exp
from losses.box_utils import (match_ious, bbox_overlaps_iou,
                              bbox_overlaps_giou, bbox_overlaps_diou,
                              bbox_overlaps_ciou, decode)
...
class IouLoss(nn.Module):
    ...
    def __init__(self,pred_mode = 'Center',size_sum=True,
                 variances=None,losstype='Giou'):
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, cfg,
        ...
        self.loss = loss_name
        #TODO 默认的loss类型为GIOU
        self.gious = IouLoss(pred_mode = 'Center',size_sum=True,
    ...
    def forward(self, predictions, targets):
        ...
        # TODO match priors (default boxes) and ground truth boxes
        ...
        for idx in range(batch):
            ...
            if self.loss == 'SmoothL1':
                #TODO 对匹配的gt box进行编码，编码anchor box和gt box之间的中心偏移以及高宽比
                match(self.threshold, truths,
                      defaults, self.variance, labels,
                      loc_t, conf_t, idx)
            else:
                match_ious(self.threshold, truths,
                           defaults, self.variance, labels,
                      loc_t, conf_t, idx)
        ...
        # print('batch_conf.shape: {}'.format(batch_conf.size()))
        # print('conf_t.shape: {}'.format(conf_t.size()))
        loss_c = log_sum_exp(batch_conf) - batch_conf.gather(
            dim = 1, index = conf_t.view(-1, 1)
        )
