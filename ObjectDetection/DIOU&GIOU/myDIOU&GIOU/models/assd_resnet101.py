...
def build_conf(cfg_i, num_blocks, num_classes):
    ...
    for i in range(len(num_blocks)):
        layers += [
            torch.nn.Conv2d(
                cfg_i[i], num_blocks[i]*num_classes,
...
def nms(boxes, scores, nms_thresh=0.5, top_k=200):
    ...
    #todo 首先对得分进行升序排序，排序之后选择预测概率最大的前topk个
    v, idx = scores.sort(0)
    idx = idx[-top_k:]
    yy1 = boxes.new()  # TODO create a new tensor of the same type
    xx1 = boxes.new()
    yy2 = boxes.new()
    xx2 = boxes.new()
    h = boxes.new()
    ...
    while idx.numel()>0:
        ...
        if idx.size(0) == 1:
            break
        #TODO 并且将得分最大的从idx中去掉
        idx = idx[:-1]

        # doing about remains...
        # TODO select the remaining boxes
...
class Detect(object):
    ...
    def __call__(self, locs, confs, priors):
        ...
        self.output.zero_()
        # p_confs: 21 x num_priors todo 这里的num_priors应该是所有层的以及每一个预测特征点位置anchor box和，比如原有的SSD为8732
        if num_batch == 1:
            p_confs = confs.t().contiguous().unsqueeze(0)
        else:
            p_confs = confs.view(num_batch, num_priors,
                                 self.num_classes).transpose(2, 1)
            self.output.expand_(num_batch,
                                self.num_classes, self.top_k, 5)

        locs = locs.data.cpu()
        priors = priors.data.cpu()
        p_confs = p_confs.data.cpu()

        # TODO 遍历batch 并Decoding...
        for i in range(num_batch):
            #TODO 根据训练之前的编码公式，得到预测结果的解码[xmin,ymin,xmax,ymax]
            decoded_boxes_i = decode(locs[i], priors,
                                     torch.Tensor(self.variance))
            p_conf_i = p_confs[i].clone()
            #TODO 遍历每一个类别并执行NMS过滤掉重叠的框
            for cl in range(1, self.num_classes):
                ...
                keep,boxes,scores = myNMS(boxes=p_boxes_i_cl, scores=p_conf_i_cl,
                             iou_threshold=self.nms_thresh,topk=200)
                self.output[i, cl, :len(keep)] = torch.cat(
                    (
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        f = hw_flattern(f) #TODO [B,C,HW]
        f = torch.transpose(f, dim0=1, dim1=2)    # [bs,N,c']
        g = hw_flattern(g)              # [bs,c',N]
        h = hw_flattern(h)              # [bs,c,N]
        h = torch.transpose(h, dim0=1, dim1= 2)    # [bs,N,c]

        s = torch.matmul(f,g)           # [bs,N,N]
        beta = F.softmax(s, dim=-1)

        o = torch.matmul(beta,h)        # [bs,N,c]
        o = torch.transpose(o, dim0=1, dim1 = 2)
        o = o.view(x.shape)
        x = o + x
        return x
...
class ASSD_ResNet101(nn.Module):
    #TODO num_blocks就是每一个输出层对应的anchor box数量
    def __init__(self, num_classes, num_blocks,
        ...
        #TODO self.L2Norm = L2Norm(n_channels=512, scale=20)
        self.extra_layers = nn.ModuleList(add_extras(inplances=layer_cfg['extra'], batch_norm=True))
        self.conf_layers = nn.ModuleList(
            build_conf(
                cfg_i=layer_cfg['pred'],
        ...
        #TODO 生成anchor box
        self.prior_boxes = PriorBox(self.device)
    ...
    def forward(self, x, phase=None):
        ...
        feat1 = F.interpolate(self.fusion_layers[1](feat1),size=(65,65),mode='bilinear')
        feat2 = F.interpolate(self.fusion_layers[2](feat2),size=(65,65),mode='bilinear')
        feat[0] = F.relu(
            self.fusion_conv(
                self.fusion_bn(
                    torch.cat([feat0, feat1, feat2], dim=1)
                )
            )
        )
        ##################################################################
...
def demo():
    ...
    model = ASSD_ResNet101(num_classes=cfg.num_classes,
                            num_blocks=cfg.mbox,
                            top_k=cfg.top_k,
    ...
    x = torch.zeros(size=(2,3,513,513)).to(device)
