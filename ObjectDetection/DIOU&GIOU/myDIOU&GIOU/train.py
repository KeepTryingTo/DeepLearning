...
from models.assd_resnet101 import ASSD_ResNet101
from losses.dec_loss import DEC_loss
from losses.iou_loss import DIOULoss,CIOULoss,GIOULoss
import torch.optim as optim
from datasetes.dataset_pascal import PASCALVOC
from datasetes import transforms
from datasetes.viewDatasets import viewDatasets_DEC
import os
import numpy as np
import cv2
import time
...
class DEC_Module(object):
    ...
    def train(self, vis=False):
        ...
        criterion = MultiBoxLoss(cfg=cfg, overlap_thresh=0.5,
                     ...
                     loss_name='Giou')
        #TODO IOU Loss
        # criterion = DEC_loss(num_classes=cfg.num_classes, variances=cfg.variance,
        #                      device=device)
        ...
        for epoch in range(total_epoch):
            ...
            for phase in ['train','val']:
                if phase == 'train':
                    ...
                    loader = tqdm(dset_loaders,desc="training...")
                    per_batch_count = 0
                    for step,data in enumerate(loader):
                        inputs, target = data
                        inputs = inputs.to(device)
                        per_batch_count += inputs.size()[0]
                        ...
                        if step != 0:
                            loader.set_postfix(
                                running_loss=running_loss / per_batch_count
                            )
                    ...
                    if epoch % 5 == 0:
                        torch.save(self.model.state_dict(),
                                   os.path.join('weights', 'giou_{:d}_{:.4f}_model.pth'.format(epoch, epoch_loss)))
                    torch.save(self.model.state_dict(),
    ...
    def eval(self, device, eval_model, dsets):
        ...
        for i in range(num_imgs):
            ...
            _t['im_detect'].tic()
            #TODO 返回对应的预测box以及confidence
            detections = eval_model(x, 'test')
            detect_time = _t['im_detect'].toc(average=False)

            # TODO ignore the background boxes
            for j in range(1, detections.size(1)):
                dets = detections[0, j, :]
                #TODO 选择那些置信度大于0的检测结果
                mask = dets[:, 0].gt(0.).expand(5, dets.size(0)).t()
                dets = torch.masked_select(dets, mask).view(-1, 5)
                if dets.shape[0] == 0:
                    continue
    ...
    def eval_single(self):
        ...
        det_file = os.path.join(output_dir, 'detection.pkl')
        print('Detecting bounding boxes...')
        all_boxes = [[[] for _ in range(num_imgs)] for _ in range(cfg.num_classes)]
