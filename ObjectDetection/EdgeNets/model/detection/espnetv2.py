...
class ESPNetv2SSD(nn.Module):
    ...
    def __init__(self, args, extra_layer):
        ...
        # TODO 去掉backbone的分类层 delete the classification layer
        ...
        #TODO  对应选择的backbone模型通道数 retrive the basenet configuration
        base_net_config = self.basenet.config
        config = base_net_config[:4] + [base_net_config[5]]

        # TODO 对于目标检测，需要在backbone后面继续添加卷积进行分类和回归操作 add configuration for SSD version
        config += [512, 256, 128]
    ...
    def forward(self, x, is_train=True):
        ...
        # TODO Detection network's extra layers
