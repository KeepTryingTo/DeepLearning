...
def main(args):
    ...
    if args.live:
        main_live(predictor=predictor, model=model,
                  object_names=object_names,
                  stream_url=args.stream, device=device)
    else:
...
def main_images(predictor, model, object_names, in_dir, out_dir, device='cuda'):
    ...
    with torch.no_grad():
        for img_name in file_names:
            ...
            new_file_name = os.path.join(out_dir, os.path.basename(img_name))
            print('save dir: {}'.format(new_file_name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--im-dir', default='./images', type=str, help='Image file')
    parser.add_argument('--save-dir', default='./outputs/', type=str, help='Directory where results will be stored')
