...
# to espnet_images, where pixel values encode ground truth classes.
#
# The Cityscapes downloads already include such espnet_images
...
Label = namedtuple( 'Label' , [
    ...
    'id'          , # An integer ID that is associated with this label.
                    # The IDs are used to represent the label in ground truth espnet_images
                    # An ID of -1 means that this label does not have an ID and thus
                    # is ignored when creating ground truth espnet_images (e.g. license plate).
                    # Do not modify these IDs, since exactly these IDs are expected by the
                    # evaluation server.

    'trainId'     , # Feel free to modify these IDs as suitable for your method. Then create
                    # ground truth espnet_images with train IDs, using the tools provided in the
                    # 'preparation' folder. However, make sure to validate or submit results
                    # to our evaluation server using the regular IDs above!
                    # For trainIds, multiple labels might have the same ID. Then, these labels
                    # are mapped to the same class in the ground truth espnet_images. For the inverse
    ...
    'categoryId'  , # The ID of this category. Used to create ground truth espnet_images
                    # on category level.
