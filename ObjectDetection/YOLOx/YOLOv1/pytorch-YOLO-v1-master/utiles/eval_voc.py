...
if __name__ == '__main__':
    ...
    model = vgg16_bn()
    model.load_state_dict(torch.load('../weights/vgg_epoch_218_best.pth'))
    model.eval()
    model = model.to(device)
    count = 0
    for image_path in tqdm(image_list):
        result = predict_gpu(model,image_path,
                             root_path='/data1/KTG/myDataset/VOC/Image07_12/',
                             device=device) #result[[left_up,right_bottom,class_name,image_path],]
    ...
    voc_eval(preds,target,VOC_CLASSES=VOC_CLASSES,use_07_metric=True)
