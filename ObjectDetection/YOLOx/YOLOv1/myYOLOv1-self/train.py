...
def main():
    ...
    model = loadModel(
        pretrained=True,resume=r'weights/epoch12_7.79_darknet_losses_t_best_model.pth.tar'
    ).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        trainloader12 = tqdm(iterable=trainLoader12,desc="darknet train is:",leave=True)
        val_loss_epoch = 0
        model.train()
        train_epoch(trainloader12,optimizer,model,loss_fn,lr_scheduler,epoch)
        trainloader07 = tqdm(iterable=trainLoader07, desc="darknet train is:", leave=True)
        train_epoch(trainloader07,optimizer,model,loss_fn,lr_scheduler,epoch)

        ################# evaluate #############################
        model.eval()
        if epoch % VAL_STEP == 0:
            loader = tqdm(iterable=valLoader, desc="darknet val is:", leave=True)
            ...
            if min_val_loss > val_loss_epoch_avg:
                ...
                torch.save(state,f'weights/epoch{epoch}_{round(min_val_loss,3)}_darknet_losses_t_best_model.pth.tar')
