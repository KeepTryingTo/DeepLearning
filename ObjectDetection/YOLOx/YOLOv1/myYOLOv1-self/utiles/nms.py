...
def convert_cellboxes(
        predictions, S=7,B = 2,
        num_classes = 20,conf_threshold = 0.1,
        iou_threshold = 0.1
):
    """
    ...
    predictions = predictions.to("cpu").squeeze(0)
    # print("_predictions.shape: {}".format(predictions.shape))

    ###############################################################################
    scores = torch.cat([
        predictions[...,num_classes + 4].unsqueeze(dim = 2),
        predictions[...,num_classes + 9].unsqueeze(dim = 2)
    ],dim=2)
    #根据confidence进行初筛
    mask = scores > conf_threshold
    mask0 = (scores == scores.max())
    #gt(0)获取score大于0的
    mask = (mask + mask0).gt(0)
    boxes = []
    cls_indexes = []
    probs = []
    cell_size = 1 / S
    for i in range(S):
        for j in range(S):
            for b in range(B):
                if mask[i, j, b] == 1:
                    box = predictions[i, j, num_classes + b * 5:num_classes + b * 5 + 4]
                    confidence = torch.FloatTensor([predictions[i, j, num_classes + b * 5 + 4]])
                    cxcy = torch.FloatTensor([j,i]) * cell_size  # cell左上角  up left of cell
                    #TODO (cx,cy) => (cx + j,cy + i);return cxcy relative to image
                    box[0],box[1] = box[0] * cell_size + cxcy[0],box[1] * cell_size + cxcy[1]
                    #TODO [cx,cy,w,h] => [xmin,ymin,xmax,ymax]
                    box_xy = cxcywhToxyxy(boxes=box)
                    # print('softmax pre: ',predictions[i, j, :num_classes])
                    # predictions[i, j, :num_classes] = predictions[i,j,:num_classes].softmax(dim = -1)
                    # print('softmax post: ',predictions[i, j, :num_classes])
                    max_prob, cls_index = torch.max(predictions[i, j, :num_classes], dim = -1)
                    #根据Pr(class) * IOU进行第二遍筛选
                    if float((confidence * max_prob)[0]) > conf_threshold:
                        boxes.append(box_xy.view(1, 4))
                        cls_indexes.append(torch.tensor([cls_index]))
                        probs.append(confidence * max_prob)
    ...
    if len(boxes) == 0:
        ...
        cls_indexes = torch.cat(cls_indexes, dim = 0)  # (n,)

    ###############################################################################
    return boxes,probs,cls_indexes,is_exist_object

#计算non_max_suppression的方式一
def nms(
        boxes,probs,cls_indexes,
        iou_threshold = 0.1
):
    """
    NMS主要是去掉冗余的框，比如重叠的框
