...
class YOLOv2D19(nn.Module):
    def __init__(self, device, input_size=None, num_classes=20,
                 trainable=False, conf_thresh=0.001,
                 nms_thresh=0.5, anchor_size=None):
    ...
    def inference(self, x):
        ...
        if isinstance(self.input_size,list):
            ...
            self.input_size = torch.tensor([w,h,w,h],
    ...
    def forward(self, x, target=None):
        if not self.trainable:
            ...
            # TODO 计算预测结果和gt box之间的IOU set conf target
