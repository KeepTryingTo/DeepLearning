...
def train_epoch(trainloader,model,optimizer,loss_fn,lr_scheduler,epoch):
    ...
    for step, data in enumerate(trainloader):
        ...
        img, loc_targets, cls_targets = (img.to(DEVICE),
                                         loc_targets.to(DEVICE),
...
# def loadCOCODataset():
#     # 加载数据集
#     dataset_train = build(image_set='val', coco_path=COCO_PATH)
#     dataset_val = build(image_set='val', coco_path=COCO_PATH)
#     trainLoader = DataLoader(
...
def main():
    ...
    loss_fn = Yolov2Loss(
        img_size=IMG_SIZE, S=S, B=B, num_classes=VOC_NUM_CLASSES,
    ...
    for epoch in range(start_epoch,EPOCHS):
        ...
        model.eval()
        if epoch % VAL_STEP == 0:
            loader = tqdm(iterable=valLoader, desc="v is:", leave=True)
            ...
            if min_val_loss > val_loss_epoch_avg:
                ...
                root_path = r'./weights'
                torch.save(state, f'{root_path}/{round(min_val_loss, 3)}_vgg16_losses_t_best_model.pth.tar')
