...
def voc_eval(
        ...
        iou_threshold=0.005,use_07_metric=False
):
    '''
...
def predict_gpu(model,image_name,root_path=''):
    ...
    boxes,probs,cls_indexs,_ =  convert_cellboxes(
        predictions=pred,conf_threshold=0.05
    )
    boxes,probs,cls_indexs = nms(boxes=boxes,scores=probs,labels=cls_indexs)
...
if __name__ == '__main__':
    ...
    dataset = VOCDataSet(voc_root=voc_path,year = '2007',
    ...
    model = YOLOv2ResNet(B=B, S=S, C=VOC_NUM_CLASSES)
    # model = Darknet19(num_classes=VOC_NUM_CLASSES)
    # model = vgg16(num_classes=VOC_NUM_CLASSES)
    checkpoint = torch.load(r'../weights/6.71_resnet_losses_t_best_model.pth.tar',map_location='cpu')
    model.load_state_dict(checkpoint['model'])
    model.eval()
    model.to(device)
    print('model is loaded done ...')

    root = os.path.join(voc_path, f"VOC{2007}")
    ...
    voc_eval(preds,target,VOC_CLASSES=VOC_CLASSES,iou_threshold=0.5,use_07_metric=True)

"""
resnet: ---map 0.28138799042802237---
