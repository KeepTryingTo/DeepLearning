...
def predict_gpu(model,image_name,root_path=''):
    ...
    boxes,probs,cls_indexs,_ =  convert_cellboxes(
        predictions=pred,conf_threshold=0.05,device = DEVICE
    )
    boxes,probs,cls_indexs = nms(boxes=boxes,
                                 scores=probs,
                                 labels=cls_indexs
                                 )
...
if __name__ == '__main__':
    ...
    dataset = VOCDataSet(voc_root=voc_path,train_set='val.txt')
    ...
    checkpoint = torch.load(r'../weights/1.898_darknet53_losses_t_best_model.pth.tar',map_location='cpu')
    model.load_state_dict(checkpoint['model'])
    model.eval()
    model.to(device)

    root = os.path.join(voc_path, f"VOC{2012}")
