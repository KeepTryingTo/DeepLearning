...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=20,S = 7,B = 2):
        self.inplanes = 64
        ...
        self.out_dim = num_classes + B * 5
        # self.conv_end = nn.Conv2d(256, self.out_dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv_end = nn.Conv2d(256, self.out_dim, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn_end = nn.BatchNorm2d(self.out_dim)
...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        for key in list(checkpoint.keys()):
            if key.startswith('layer5') or key.startswith('fc'):
                del checkpoint[key]
