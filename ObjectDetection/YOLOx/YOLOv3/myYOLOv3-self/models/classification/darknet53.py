...
class YOLOv3(torch.nn.Module):
    def __init__(self,in_channels = 3,num_classes = 20):
        ...
        ######################  classifier head  #############
        self.down26to13 = nn.Sequential(
            nn.Conv2d(in_channels=(self.num_classes + 5) * 3,out_channels=512,kernel_size=1),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.down52to26to13 = nn.Sequential(
            nn.Conv2d(in_channels=3 * (self.num_classes + 5),out_channels=512,kernel_size=3,stride=2,padding=1),
            nn.BatchNorm2d(num_features=512),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2,stride=2)
        )
        self.avgpool = nn.AdaptiveAvgPool2d(output_size=1)
        self.conv = nn.Conv2d(in_channels=(self.num_classes + 5) * 3,out_channels=512,kernel_size=1)
        self.classifier = nn.Sequential(
            nn.Linear(in_features=512*3, out_features=self.num_classes)
        )
    ...
    def forward(self,x):
        ...
        down26to13 = self.down26to13(outputs[1])
        down52to26to13 = self.down52to26to13(outputs[2])
        outputs[0] = self.conv(outputs[0])
        out = torch.cat([outputs[0],down26to13,down52to26to13],dim = 1)
        out = self.avgpool(out).view(b,-1)
