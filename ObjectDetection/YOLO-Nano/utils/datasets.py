...
class ListDataset(Dataset):
    ...
    def annotation_build(self,img_key):
        ...
        label_matrix = np.asarray(label_matrix).astype(float)
        return label_matrix
    ...
    def __getitem__(self, index):
        ...
        if self.use_mix:
            ...
            #TODO gt_mix_index用于区别混合的两个batch中哪些原图，哪些是混合的图
            img,gtboxes, gtlabels, gtscores,gt_mix_index = image_mixup(img,
                                                                       img_target,
                                                                       img_label,
                                                                       img_scores,
                                                                       mix_img,
        ...
        if (label_matrix.shape[0] != 0):
            ...
            #TODO  Extract coordinates for unpadded + unscaled image
            x1 = w_factor * boxes[:, 1]
            y1 = h_factor * boxes[:, 2]
            x2 = w_factor * (boxes[:, 1] + boxes[:, 3])
            y2 = h_factor * (boxes[:, 2] + boxes[:, 4])
            # TODO 图像被填充了，那么box也需要改变 Adjust for added padding
            x1 += pad[0]
            y1 += pad[2]
            x2 += pad[1]
            y2 += pad[3]
    ...
    def collate_fn(self, batch):
        ...
        # TODO Add sample index to targets
        ...
        if len(targets) !=0:
            ...
            # TODO Selects new image size every tenth batch
        if self.multiscale and self.batch_count % 10 == 0:
            self.img_size = random.choice(range(self.min_size, self.max_size + 1, 32))
        # TODO Resize images to input shape
