...
class YOLOLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, cuda=True):
        ...
        # TODO Calculate offsets for each grid 计算网格序列
        self.grid_x = torch.arange(g).repeat(g, 1).view([1, 1, g, g]).type(FloatTensor)
        self.grid_y = torch.arange(g).repeat(g, 1).t().view([1, 1, g, g]).type(FloatTensor)
        #TODO 根据当前输出特征图大小和对应下采样步长缩放anchor的尺度
    ...
    def forward(self, x, targets=None, img_dim=None,img_scores=None,gt_mix_index = None):
        ...
        # TODO If grid size does not match current we compute new offsets
        if grid_size != self.grid_size:
            self.compute_grid_offsets(grid_size, cuda=x.is_cuda)

        # TODO 根据网格以及anchor高宽来对预测结果进行解码操作 Add offset and scale with anchors
        ...
        if targets is None:
            ...
            #TODO from uint8 convert to bool
            if obj_mask_1 is not None and obj_mask_1.dtype == torch.uint8:
                obj_mask_1 = obj_mask_1.bool()
            if obj_mask_2 is not None and obj_mask_2.dtype == torch.uint8:
                obj_mask_2 = obj_mask_2.bool()

            if obj_mask is not None and obj_mask.dtype == torch.uint8:
                obj_mask = obj_mask.bool()
            if noobj_mask is not None and noobj_mask.dtype == torch.uint8:
            ...
            #TODO 计算置信度损失
            loss_conf_obj_1 = torch.mean(self.bce_loss(pred_conf[obj_mask_1],
                                                       tconf[obj_mask_1])*sum_weights1[obj_mask_1])
            if obj_mask_2 is not None:
                loss_conf_obj_2 = torch.mean(self.bce_loss(pred_conf[obj_mask_2],
                                                           tconf[obj_mask_2])*sum_weights2[obj_mask_2])
            else:
                loss_conf_obj_2 = 0.
            ...
            # print('pred cls.shape: {}'.format(pred_cls[obj_mask_1].size()))
            # print('target cls.shape: {}'.format(tcls[obj_mask_1].size()))
            # print('sum weight1.shape: {}'.format(sum_weights1[obj_mask_1].size()))

            nB,nA,nG,_,nC = tcls.size()
            sum_weights1 = sum_weights1.unsqueeze(dim = -1).expand(nB,nA,nG,nG,nC)
            loss_cls_1 = torch.mean(self.bce_loss(pred_cls[obj_mask_1],
                                                  tcls[obj_mask_1])*sum_weights1[obj_mask_1])
            if obj_mask_2 is not None:
                sum_weights2 = sum_weights2.unsqueeze(dim = -1).expand(nB,nA,nG,nG,nC)
                loss_cls_2 = torch.mean(self.bce_loss(pred_cls[obj_mask_2],
                                                      tcls[obj_mask_2])*sum_weights2[obj_mask_2])
            else:
                loss_cls_2 = 0.
