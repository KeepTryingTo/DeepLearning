...
def inference(
    ...
    model.eval()
    # Get dataloader
    dataset = InferenceDataset(path, img_size=img_size,
    ...
    vider_write = cv2.VideoWriter(osp.join("./result", "{}.mp4".format(video_id)),
                                  fourcc, 30.0,
                                  VIDEO_SIZE[video_id])
    for batch_i, (img_id, imgs, pads) in enumerate(tqdm.tqdm(dataloader,
