...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # TODO 加载预训练模型 Model（以yolov5作为预训练模型）
    ...
    # TODO 或则下采样步长以及训练和测试图像大小 Image sizes
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test,
                                       total_batch_size, gs, opt,  # testloader
    ...
    # TODO 针对预训练在COCO上的模型，这里需要进行一定的微调 Model parameters
    hyp['cls'] *= nc / 80.  # scale coco-tuned hyp['cls'] to current dataset
    model.nc = nc  # attach number of classes to model
    model.hyp = hyp  # attach hyperparameters to model
    model.gr = 1.0  # iou loss ratio (obj_loss = 1.0 or iou)
    model.class_weights = labels_to_class_weights(labels=dataset.labels, nc=nc).to(device) * nc  # attach class weights
    model.names = names
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls',
                                           'landmark', 'total', 'targets', 'img_size'))
        if rank in [-1, 0]:
