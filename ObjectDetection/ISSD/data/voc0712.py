...
class VOCDetection(data.Dataset):
    ...
    def __init__(self, root, image_sets,
                 preproc=None, target_transform=None,
        ...
        for cls_ind, cls in enumerate(VOC_CLASSES):
            ...
            with open(filename, 'wt') as f:
                ...
                for im_ind, index in enumerate(self.ids):
                    index = index[1]
                    #TODO 根据类别id和图像索引id获得检测box
                    dets = all_boxes[cls_ind][im_ind]
                    if dets == []:
                        continue
                    for k in range(dets.shape[0]):
                        #TODO 保存index，confidence，box
                        #TODO 将类别索引也写入到结果文件中
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        print("VOCDetection do python eval: {}".format(rootpath))
        ...
        aps = []
        # The PASCAL VOC metric changed in 2010 TODO 判断使用07还是12数据集
        ...
        for i, cls in enumerate(VOC_CLASSES):
            ...
            if cls == '__background__':
                continue
            #TODO 获得用于保存预测当前类别的文件名
            filename = self._get_voc_results_file_template().format(cls)
            # print('filename: {}'.format(filename))
            # print('imagesetfile: {}'.format(imagesetfile))
            #TODO 获得当前类别的召回率，准确率和AP值
