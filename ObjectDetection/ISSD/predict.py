...
parser.add_argument('-v', '--version', default='ISSD_vgg',help='ISSD_vgg ,'
                                            ' ISSD_mobile version.')
...
parser.add_argument('-m', '--trained_model',
                    default=r'runs/ISSD_vgg_VOC_epoches_225.pth',
                    type=str, help = 'Trained state_dict file path to open')
parser.add_argument('--save_folder', default='./outputs_nmw', type=str,
    ...
    for imgName in img_list:
        ...
        for j in range(1, num_classes):
            ...
            if is_nmw:
                ...
                keep, count, c_bboxes = nmw(r_boxes, r_scores,
                                            overlap=iou_thresh,
                                            top_k=topk)
            else:
                from layers.nms import nms
                keep, _ = nms(r_boxes, r_scores,
                              overlap=iou_thresh,
                              top_k=topk)
...
if __name__ == '__main__':
    ...
    detector = Detect(num_classes=num_classes,
                      bkg_label=0, cfg=cfg,device=device)
