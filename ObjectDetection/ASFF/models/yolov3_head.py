...
from utils1.utils2 import bboxes_iou
import numpy as np
from models.utils_loss import *
from models.network_blocks import *

class YOLOv3Head(nn.Module):
    def __init__(self, anch_mask, n_classes, stride, in_ch=1024,
                 ignore_thre=0.7, label_smooth = False,
                 rfb=False, sep=False):
        ...
        self.Feature_adaption=FeatureAdaption(in_ch=in_ch, out_ch=in_ch,
        ...
        #TODO 下采样步长以及是否使用标签平滑
        self.stride = stride
        self._label_smooth = label_smooth

        self.all_anchors_grid = self.anchors
        #TODO 根据当前层来得到对应的anchor box（每一层为三个不同尺寸的anchor box）
    ...
    def forward(self, xin, labels=None):
        ...
        #TODO 将anchor的维度改变为指定的形状
        w_anchors = dtype(np.broadcast_to(np.reshape(
            masked_anchors[:, 0], (1, self.n_anchors-1, 1, 1)),
            [batchsize, self.n_anchors-1, fsize, fsize]))
        h_anchors = dtype(np.broadcast_to(np.reshape(
            masked_anchors[:, 1], (1, self.n_anchors-1, 1, 1)),
            [batchsize, self.n_anchors-1, fsize, fsize]))

        #TODO 指定预测的anchor高宽中心坐标为0
        ...
        #TODO 根据预测的anchor高宽偏移来对anchor box高宽进行调整
        ...
        """
        修改 anchors_based 和 anchors_free 的值会导致 pred_anchors 
        中对应元素的值发生改变，因为 anchors_based 和 anchors_free 
        是基于 pred_anchors 的共享视图。
        """
        ...
        if not self.training:
            ...
            pred = output.clone()
            """
            np.r_ 是 NumPy 库中的一个工具，主要用于快速连接数组。
            它提供了一种简单的语法来拼接多个数组，无论是水平（列方向）还是垂直（行方向）拼接
            import numpy as np  
            # 使用 np.r_ 生成一个范围的数组并拼接  
            result = np.r_[1:5, np.array([6, 7]), 8:10]  
            print(result)  # 输出: [1 2 3 4 6 7 8 9]
            """
            #TODO 根据YOLOV3对中心坐标以及高宽的变换方式
        ...
        #TODO training for anchor prediction
        if self.training:
            target = torch.zeros(batchsize, self.n_anchors,fsize, fsize, n_ch).type(dtype)
            l1_target = torch.zeros(batchsize, self.n_anchors,fsize, fsize, 4).type(dtype)
            tgt_scale = torch.zeros(batchsize, self.n_anchors,fsize, fsize, 4).type(dtype)
            ...
            for b in range(batchsize):
                ...
                #TODO 统计和gt  box最大匹配的那些anchor box
                ...
                pred_box = pred_boxes[b].contiguous()
                #TODO 计算预测box和gt box之间的IOU以及统计和预测box最佳匹配的那些gt box
                pred_ious = bboxes_iou(pred_box.view(-1,4),truth_box, xyxy=False)
                pred_best_iou, _= pred_ious.max(dim=1)
                pred_best_iou = (pred_best_iou > self.ignore_thre) #TODO 过滤掉那些冗余的框
            ...
            loss_anchor = self.iou_wh_loss(pred_anchors[...,:4][anchorcoord_mask],
                                           target[...,:4][anchorcoord_mask]).sum()/batchsize

            #Prediction loss
            coord_mask = coord_mask>0
            loss_iou = (tgt_scale[coord_mask][...,0] * self.iou_loss(pred[..., :4][coord_mask],
                                  target[..., :4][coord_mask])).sum() / batchsize
            tgt_scale = tgt_scale[...,:2]
            loss_xy = (tgt_scale*self.bcewithlog_loss(output[...,:2],
                                                      l1_target[...,:2])).sum() / batchsize
            loss_wh = (tgt_scale*self.l1_loss(output[...,2:4],
                                              l1_target[...,2:4])).sum() / batchsize
            loss_l1 = loss_xy + loss_wh
            loss_obj = (obj_mask*(self.bcewithlog_loss(output[..., 4],
                                                       target[..., 4]))).sum() / batchsize
            loss_cls = (cls_mask*(self.bcewithlog_loss(output[..., 5:],
                                                       target[..., 5:]))).sum()/ batchsize
