...
from torchvision.ops import deform_conv2d,deform_conv
from utils1.DCN.modules.deform_conv2d import DeformConv2d
...
class SPPLayer(nn.Module):
    ...
    def forward(self, x):
        x_1 = x
        x_2 = F.max_pool2d(x, kernel_size = 5, stride=1, padding=2)
        x_3 = F.max_pool2d(x, kernel_size = 9, stride=1, padding=4)
        x_4 = F.max_pool2d(x, kernel_size = 13, stride=1, padding=6)
...
class DropBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        torch.bernoulli 是 PyTorch 中的一个函数，用于从伯努利分布中抽取样本。
            伯努利分布是一个简单的离散概率分布，通常用于二元（0 或 1）数据生成，其中概率为 p。
        """
    ...
    def __init__(self, in_ch, out_ch, n_anchors, rfb=False, sep=False):
        ...
        if sep:
            ...
            self.conv_offset = nn.Conv2d(in_channels=2*n_anchors, 
                    ...
                    groups = n_anchors, kernel_size=1,
                    stride=1,padding=0)
            self.dconv = DeformConv2d(in_channels=in_ch, out_channels=out_ch,
                    kernel_size=3, stride=1,
                    padding=1, deformable_groups=n_anchors)
            # self.dconv = deform_conv2d(
