...
from models.yolov3_head import YOLOv3Head
...
class YOLOv3(nn.Module):
    ...
    def __init__(self, num_classes = 80, ignore_thre=0.7,
                 label_smooth = False,
                 rfb=False, vis=False, asff=False):
        """
        ...
        #TODO 构建YOLOv3网络结构
        self.module_list = build_yolov3_modules(num_classes, ignore_thre,
                                                label_smooth, rfb)

        #TODO 其中rfb表示是否采用RFBlock论文提出的方法（之前已经讲过了）
    ...
    def forward(self, x, targets=None, epoch=0):
        ...
        for l in range(3):
            ...
            if self.vis:
                #TODO
                fused, weight, fuse_f = fusion(route_layers[2],
                                               route_layers[3],
                                               route_layers[4])
                fuse_wegihts.append(weight)
                fuse_fs.append(fuse_f)
            else:
                fused = fusion(route_layers[2],
                               route_layers[3]
                               ,route_layers[4])
            #TODO 如果是训练的话就计算对应的损失
            if train:
...
def demo():
    ...
    x = torch.zeros(size=(1,3,608,608)).to(device)
    model = YOLOv3(num_classes=20).to(device)
    model.eval()
    outs = model(x)
    print('outs.shape: {}'.format(outs.size()))
    # for key,item in outsLoss.items():
