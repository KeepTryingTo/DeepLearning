...
def create_yolov3_mobilenet_v2(num_classes, width_mult=1.0,
...
class YOLOv3(nn.Module):
    ...
    """
    def __init__(self, num_classes = 80, ignore_thre=0.7,
                 label_smooth = False,
                 rfb=False, vis=False,
                 asff=False):
        """
        ...
        if asff:
            ...
            self.level_0_conv =add_conv(in_ch=512, out_ch=1024, ksize=3,
                                        stride=1,leaky=False)

        self.level_0_header = YOLOv3Head(anch_mask=[6, 7, 8],
                                         n_classes=num_classes, stride=32, in_ch=1024,
                              ignore_thre=ignore_thre,
                                         label_smooth = label_smooth, rfb=rfb, sep=True)

        if asff:
            self.level_1_conv =ASFFmobile(level=1,rfb=rfb,vis=vis)
        else:
            self.level_1_conv =add_conv(in_ch=256, out_ch=512, ksize=3,
                                        stride=1,leaky=False)

        self.level_1_header = YOLOv3Head(anch_mask=[3, 4, 5],
        ...
        if asff:
            ...
            self.level_2_conv =add_conv(in_ch=128, out_ch=256, ksize=3,
                                        stride=1,leaky=False)

        self.level_2_header = YOLOv3Head(anch_mask=[0, 1, 2],
                                         n_classes=num_classes, stride=8, in_ch=256,
                                         ignore_thre=ignore_thre,
                                         label_smooth = label_smooth, rfb=rfb, sep=True)
        self.asff = asff
