from utils1.utils2 import *
# from utils1.cocoapi_evaluator import COCOAPIEvaluator
from utils1.voc_evaluator import VOCEvaluator
from utils1 import distributed_util
from utils1.distributed_util import reduce_loss_dict
...
# import apex
from utils1.fp16_utils.fp16_optimizer import FP16_Optimizer
...
def parse_args():
    ...
    parser.add_argument('--cfg', type=str, default='config/yolov3_baseline.cfg',help='config file. see readme')
    parser.add_argument('-d', '--dataset', type=str,default='VOC', help='COCO or VOC dataset')
    parser.add_argument('--n_cpu', type=int, default=8,help='number of workers')
    parser.add_argument('--distributed', dest='distributed', action='store_true', default=False,help='distributed training')
    parser.add_argument('--local_rank', type=int, default=0, help='local_rank')
    parser.add_argument('--ngpu', type=int, default=1,help='number of gpu')
    parser.add_argument('--start_epoch', type=int,default=0, help='start epoch')
    parser.add_argument('--eval_interval', type=int,default=5, help='interval epoch between evaluations')
    parser.add_argument('-c', '--checkpoint', type=str,default=r'./weights/darknet53_feature_mx.pth',help='pytorch checkpoint file path')
    parser.add_argument('--save_dir', type=str,default='save',help='directory where model are saved')
    ...
    parser.add_argument('--testset', dest='testset', action='store_true', default=False,help='test set evaluation')
    parser.add_argument('--half', dest='half', action='store_true', default=False,help='FP16 training')
    parser.add_argument('--rfb', dest='rfb', action='store_true', default=False,help='Use rfb block')
    parser.add_argument('--asff', dest='asff', action='store_true', default=False, help='Use ASFF module for yolov3')
    parser.add_argument('--dropblock', dest='dropblock', action='store_true', default=False,help='Use dropblock')
    parser.add_argument('--nowd', dest='no_wd', action='store_true', default=False,help='no weight decay for bias')
    parser.add_argument('--vis', dest='vis', action='store_true', default=False,help='visualize fusion weight and detection results')
    ...
    parser.add_argument('--log_dir', type=str,default='log/',help='directory where tf log are saved')
...
def main():
    ...
    root_dir = r'/home/ff/myProject/KGT/myProjects/myDataset/voc2012'
    ...
    if args.dataset == 'COCO':
        ...
        dataset = VOCDetection(root=root_dir,
    ...
    # TODO Initiate model 默认采用asff自适应特征融合机制
    if args.asff:
        save_prefix += '_asff'
        #TODO 默认采用darknet53主干模型
        if backbone == 'mobile':
    ...
    batch_sampler = YoloBatchSampler(sampler=sampler, batch_size=batch_size,
                                     drop_last=False,input_dimension=input_size)
    ...
    # optimizer setup
    # TODO 设置权重衰减 set weight decay only on conv.weight
    if args.no_wd:
    ...
    while epoch < epochs+1:
        ...
        if (epoch == 5 or (epoch == args.start_epoch and args.start_epoch > 5)) and (args.dropblock) and backbone!='mobile':
            block_size = [1, 3, 5]
            keep_p = [0.9, 0.9, 0.9]
            #TODO 重新设置DropBlock的步长，核大小以及padding大小
        ...
        # TODO 调整学习率 learning rate scheduling (cos or step)
            if epoch < burn_in:
            ...
            #torch.nn.utils1.clip_grad_norm_(model.parameters(), 10)
            ...
            if random_resize and iter_i %10 == 0 and iter_i > 0:
                ...
                input_size = dataloader.change_input_dim(multiple=tensor.item(),
                                                         random_range=None)
...
Mean AP = 0.7341
map_5095: 0.4382707757708736
map_50: 0.7340501633493848
"""