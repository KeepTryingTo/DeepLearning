...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes,
        ...
        self.num_feature_levels = num_feature_levels #TODO 默认为4层
        #TODO 得到object query向量
        if not two_stage:
            self.query_embed = nn.Embedding(num_queries, hidden_dim*2)
        #TODO 根据backbone输出的层数来定义卷积进行通道数的转换，输入到transformer编码层
        ...
        self.aux_loss = aux_loss #TODO 默认为false
        self.with_box_refine = with_box_refine #TODO 默认为false
        self.two_stage = two_stage

        #TODO 对类别检测头的权重和偏执项进行初始化处理
        prior_prob = 0.01
        ...
        if with_box_refine:#TODO 默认为false
    ...
    def forward(self, samples: NestedTensor):
        ...
        #TODO backbone的特征图以及位置编码向量
        features, pos = self.backbone(samples)

        srcs = []
        masks = []
        #todo 对backbone的输出特征图进行转换
