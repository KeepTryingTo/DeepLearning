...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        Args:
            srcs: 经过backbone和特征金字塔变换之后的特征图
            masks: mask: a binary mask of shape [batch_size x H x W], containing 1 on padded pixels
            pos_embeds:位置编码
            query_embed:object query嵌入向量
        ...
        # TODO 默认不采用两阶段算法 prepare input for decoder
        bs, _, c = memory.shape
        if self.two_stage:
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points,
        ...
        Args:
            src: backbone输出经过转换之后的特征图
            pos: 位置编码向量
            reference_points: 每一层对应的网格参考点
            spatial_shapes: 每一层对应的特征图大小
            level_start_index: 当前层开始索引
            padding_mask: 填充图像的掩码
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src,
        ...
        for lid, layer in enumerate(self.layers):
            #TODO 根据生成的参考点以及有效输入特征图区域得到最终的参考点
            ...
            # TODO hack implementation for iterative bounding box refinement
            #TODO 默认的bbox_embed = None
            if self.bbox_embed is not None:
                #TODO 根据当前的参考点以及预测结果进行调整
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        d_model=args.hidden_dim,#TODO 默认为256
        nhead=args.nheads, #TODO 默认为8
        num_encoder_layers=args.enc_layers,#TODO 默认为6层
        num_decoder_layers=args.dec_layers,
        dim_feedforward=args.dim_feedforward,
        dropout=args.dropout,
        activation="relu",
        return_intermediate_dec=True,
        num_feature_levels=args.num_feature_levels, #TODO 默认为4层
        dec_n_points=args.dec_n_points, #TODO 默认为4个参考点
        enc_n_points=args.enc_n_points,
        two_stage=args.two_stage, #TODO 默认为false
