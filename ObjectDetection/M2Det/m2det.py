...
class M2Det(nn.Module):
    ...
    def construct_modules(self,):
        ...
        self.reduce= BasicConv(in_planes=shallow_in, out_planes=shallow_out,
                               kernel_size=3, stride=1, padding=1)
        self.up_reduce= BasicConv(in_planes=deep_in, out_planes=deep_out,
    ...
    def forward(self,x):
        ...
        # TODO tum_outs is the multi-level multi-scale feature
        tum_outs = [
            getattr(self, 'unet{}'.format(1))(self.leach[0](base_feature), 'none')]
        #TODO 针对每一层的TUM，将经过所有TUM的结果保存到tum_outs中
        ...
        #TODO 将TUM所有层相同尺度的给拼接起来
        sources = [
            torch.cat(
                [
                    _fx[i-1] for _fx in tum_outs
                ],dim = 1
...
def demo():
    model = build_net(phase='train',size=320,config=r'configs/m2det320_vgg.py')
    x = torch.zeros(size = (1,3,320,320))
    loc_logits,cls_logits = model(x)
    print('loc_logits.shape: {}'.format(loc_logits.shape))
    print('cls_logits.shape: {}'.format(cls_logits.shape))
