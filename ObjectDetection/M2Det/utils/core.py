...
def get_dataloader(cfg, dataset, setname='train_sets'):
    ...
    if setname == 'train_sets':
        dataset = _Dataloader_function(
            ...
            getattr(cfg.dataset, dataset)[setname],
            _preproc
        )
    else:
        dataset = _Dataloader_function(
...
def nms_process(num_classes, i, scores,
                boxes, cfg, min_thresh,
                all_boxes, max_per_image):
    for j in range(1, num_classes): # ignore the bg(category_id=0)
        ...
        keep = nms(c_bboxes, c_scores, iou_threshold=min_thresh)
