...
class TUM(nn.Module):
    def __init__(self, first_level=True, input_planes=128,
                 is_smooth=True, side_channel=512, scales=6):
        ...
        self.layers.add_module(
            ...
            BasicConv(self.in1, self.planes, kernel_size=3, stride=2, padding=1)
        )
        for i in range(self.scales-2):
            if not i == self.scales - 3:
                self.layers.add_module(
                        '{}'.format(len(self.layers)),
                        BasicConv(self.planes, self.planes,
        ...
        self.toplayer = nn.Sequential(
            BasicConv(self.planes, self.planes, kernel_size=1, stride=1, padding=0)
        )
...
def get_backbone(backbone_name='vgg16'):
    ...
    if backbone_name=='vgg16':
        base = [64, 64, 'M', 128, 128, 'M',
                256, 256, 256, 'C', 512, 512, 512,
                'M', 512, 512, 512]
        return vgg(base, i=3, batch_norm=False)
