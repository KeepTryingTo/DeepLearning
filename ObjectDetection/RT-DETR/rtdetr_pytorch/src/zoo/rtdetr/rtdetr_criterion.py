...
from src.misc.dist import (get_world_size,
                                          is_dist_available_and_initialized)
from src.core import register
from src.misc.nested import *
from src.zoo.rtdetr.segmentation import *
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses,
                 alpha=0.2, gamma=2.0, eos_coef=1e-4,
                 num_classes=80):
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss = F.binary_cross_entropy_with_logits(src_logits,
                                                  target_score,
                                                  weight=weight,
                                                  reduction='none')
        loss = loss.mean(1).sum() * src_logits.shape[1] / num_boxes
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'boxes': self.loss_boxes, #TODO 默认采用box损失
            'masks': self.loss_masks,

            'bce': self.loss_labels_bce,
            'focal': self.loss_labels_focal,
            'vfl': self.loss_labels_vfl, #TODO 默认采用vfl损失
        }
    ...
    def get_cdn_matched_indices(dn_meta, targets):
        ...
        '''
        dn_positive_idx, dn_num_group = (dn_meta["dn_positive_idx"],
        ...
        for i, num_gt in enumerate(num_gts):
            if num_gt > 0:
                ...
                dn_match_indices.append((torch.zeros(0,
                                                     dtype=torch.int64,
                                                     device=device),
                                         torch.zeros(0,
                                                     dtype=torch.int64,
