...
def merge_predictions(predictions, slice_coordinates, orig_image_size, slice_width, slice_height, threshold=0.30):
    ...
    return np.array(merged_labels), np.array(merged_boxes), np.array(merged_scores)
def draw(images, labels, boxes, scores, thrh = 0.6, path = "",img_name = None):
...
def main(args, ):
    ...
    for imgName in os.listdir(args.im_file):
        start_time = time.time()
        img_path = os.path.join(args.im_file,imgName)
        ...
        if args.sliced:
            ...
            overlap_ratio = 0.2
            slices, coordinates = slice_image(image=im_pil,
            ...
            for i, slice_img in enumerate(slices):
                ...
                with autocast():  # Use AMP for each slice
                    output = model(slice_tensor, torch.tensor([[slice_img.size[0],
                                                                slice_img.size[1]]]).to(args.device))
            ...
            merged_labels, merged_boxes, merged_scores = merge_predictions(predictions,
                                                                           coordinates,
                                                                           (h, w),
                                                                           slice_width, slice_height)
            labels, boxes, scores = postprocess(merged_labels,
                                                merged_boxes,
                                                merged_scores)
        else:
        ...
        save_path = r'/home/ff/myProject/KGT/myProjects/myProjects/RT-DETR-main/rtdetr_pytorch_my/outputs'
        draw([im_pil],
             labels, boxes,
             scores, 0.55,
             path=os.path.join(save_path,imgName),
             img_name = imgName)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--resume', type=str,
                        default=r'/home/ff/myProject/KGT/myProjects/myProjects/RT-DETR-main/rtdetr_pytorch_my/tools/output/rtdetr_r18vd_6x_coco/checkpoint0001.pth')
    parser.add_argument('-f', '--im-file', type=str,
                        default=r'/home/ff/myProject/KGT/myProjects/myProjects/RT-DETR-main/rtdetr_pytorch_my/images')
