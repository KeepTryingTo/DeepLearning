...
from utils import (torch_distributed_zero_first, xywhn2xyxy, xyxy2xywhn,
                   xyn2xy, segments2boxes, clean_str)
from augmentations import (Albumentations, mixup,
                           letterbox, random_perspective,
                           augment_hsv, copy_paste)
...
def img2label_paths(img_paths):
    ...
    sa, sb = (os.sep + 'images' + os.sep,
              os.sep + 'labels' + os.sep)  # /images/, /labels/ substrings
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path, img_size=640, batch_size=16,
        ...
        if self.rect:
            ...
            #TODO 读取label文件图像中物体对应的坐标以及类别标签
            self.label_files = [self.label_files[i] for i in irect]
            self.labels = [self.labels[i] for i in irect]
            self.shapes = s[irect]  # wh
            ar = ar[irect]

            # TODO Set training image shapes
            shapes = [[1, 1]] * nb
            for i in range(nb):
                ari = ar[bi == i]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # TODO Load image，返回加载的图像，原图像高宽以及缩放之后的图像高宽
            ...
            #TODO 对坐标的格式进行转换，从[cx,cy,w,h] => [xmin,ymin,xmax,ymax]
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = xywhn2xyxy(x=labels[:, 1:],
                                           w = ratio[0] * w,
                                           h = ratio[1] * h,
        ...
        if self.augment:
            ...
            # TODO 是否上下翻转图像 Flip up-down
            if random.random() < hyp['flipud']:
                img = np.flipud(img)
                if nl:
                    labels[:, 2] = 1 - labels[:, 2]

            # TODO 是否左右翻转图像 Flip left-right
