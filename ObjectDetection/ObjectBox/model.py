...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            #TODO [batch，num_anchor, num_classes + 5, f_h, f_w]
...
class Model(nn.Module):
    ...
    def _forward_augment(self, x):
        ...
        for si, fi in zip(s, f):
            ...
            #TODO 对于当前尺度图像的前向传播输出
            yi = self._forward_once(xi)[0]  # forward
            # cv2.imwrite(f'img_{si}.jpg', 255 * xi[0].cpu().numpy().transpose((1, 2, 0))[:, :, ::-1])  # save
            yi = self._descale_pred(yi, fi, si, img_size)
            y.append(yi)
        return torch.cat(y, dim = 1), None  # augmented inference, train
    ...
    def _descale_pred(self, p, flips, scale, img_size):
        ...
        #TODO 默认为true
        if self.inplace:
            #TODO 根据输入模型之前的尺度变化对经过模型之后的结果进行缩放
            p[..., :4] /= scale  # de-scale
            #TODO 默认flips = 3
            if flips == 2:
...
def scale_img(img, ratio=1.0, same_shape=False, gs=32):  # img(16,3,256,416)
    ...
    if ratio == 1.0:
        ...
        return F.pad(img, pad = [0, w - s[1], 0, h - s[0]], value=0.447)  # value = imagenet mean
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, C3, SPPF]:#, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3TR, C3SPP, C3Ghost]:
            ...
            args = [c1, c2, *args[1:]]
            #TODO 如果是C3模块就重复两次
            if m in [C3]: #todo BottleneckCSP, C3TR, C3Ghost]:
