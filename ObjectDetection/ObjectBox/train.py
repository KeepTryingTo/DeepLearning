...
from utils import (labels_to_class_weights, increment_path,
                   labels_to_image_weights, init_seeds,
                   strip_optimizer, get_latest_run,
                   check_dataset, check_img_size,
                   check_file, check_suffix, set_logging,
                   one_cycle, colorstr, methods)
from plots import plot_labels
from utils import (EarlyStopping, ModelEMA, de_parallel, intersect_dicts,
...
def train(hyp, device, callbacks):
    ...
    # TODO https://pytorch.org/docs/stable/_modules/torch/optim/lr_scheduler.html#OneCycleLR
    if FLAGS.linear_lr:
    ...
    # TODO 加载数据集 Trainloader
    train_loader, dataset = create_dataloader(path=train_path, imgsz=imgsz,
                                              batch_size=batch_size // FLAGS.world_size, stride=gs,
                                              single_cls=single_cls,
                                              hyp=hyp, augment=True, cache=FLAGS.cache_images,
                                              rect=FLAGS.rect, rank=RANK,
                                              workers=workers, image_weights=FLAGS.image_weights,
                                              quad=FLAGS.quad,
    ...
    # Process 0
    if RANK in [-1, 0]:
        val_loader = create_dataloader(path=val_path, imgsz=imgsz,
                                       batch_size=batch_size // FLAGS.world_size * 2, stride=gs,
                                       single_cls=single_cls,  # testloader
                                       hyp=hyp, cache=None if noval else FLAGS.cache_images,
                                       rect=True, rank=-1,
                                       workers=FLAGS.workers, pad=0.5,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # TODO Update image weights (optional, single-GPU only)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                accumulate = max(1, np.interp(ni, xi,
                                              fp=[1, nbs / batch_size]).round())
...
all       4952      12032       0.78      0.677      0.758      0.492
"""