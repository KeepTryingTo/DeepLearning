...
def feature_visualization(x, module_type, stage,
                          n=32, save_dir=Path('runs/detect/exp')):
    """
...
def plot_images(images, targets, paths=None, fname='images.jpg',
    ...
    for i in range(i + 1):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], fill=None,
                            outline = (255, 255, 255), width=2)  # borders
        if paths:
...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil:
            ...
            if label:
                left, top, right, bottom = self.font.getbbox(label)  # text width, height
            ...
            if label:
                ...
                cv2.putText(self.im, label, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2),
                            fontFace=0, fontScale=self.lw / 3, color=txt_color,
    ...
    def text(self, xy, text, txt_color=(255, 255, 255)):
        ...
        self.draw.text((xy[0], xy[1] - h + 1),
                       text, fill=txt_color, font=self.font)
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    # seaborn correlogram
    sn.pairplot(x, corner=True, diag_kind='auto',
                kind='hist', diag_kws=dict(bins=50),
