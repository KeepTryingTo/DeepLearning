...
class SeparableConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None,
        ...
        #TODO Conv2dStaticSamePadding 不改变feature map的大小
...
def demoSeparableConvBlock():
    model = SeparableConvBlock(
        in_channels=3
    )
...
class BiFPN(nn.Module):
    ...
    def __init__(self, num_channels, conv_channels, first_time=False,
        ...
        # TODO Feature scaling layers 上采样2倍
        ...
        if self.first_time:
            ...
            self.p4_down_channel = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[1], num_channels, kernel_size=1),
                nn.BatchNorm2d(num_channels, momentum=0.01, eps=1e-3),
            )
            self.p3_down_channel = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[0], num_channels, kernel_size=1),
                nn.BatchNorm2d(num_channels, momentum=0.01, eps=1e-3),
            )
    ...
    def forward(self, inputs):
        ...
        #TODO 从EfficientD0 - EfficientD5会采用attention
        if self.attention:
    ...
    def _forward_fast_attention(self, inputs):
        ...
        # TODO Weights for P6_0 and P7_0 to P6_1
        p6_w1 = self.p6_w1_relu(self.p6_w1)
        weight = p6_w1 / (torch.sum(p6_w1, dim=0) + self.epsilon)
        # TODO Connections for P6_0 and P7_0 to P6_1 respectively
    ...
    def _forward(self, inputs):
        #TODO 由于BIFPN模块可能 重复多次，因此这里需要判断是否为第一个。
        if self.first_time:
            p3, p4, p5 = inputs
            ...
            #TODO 如果EfficientDetx > 7就使用use_p8
            if self.use_p8:
...
def demoBiFPN():
    ...
    x0 = torch.zeros(size=(1,40,64,64))
    x1 = torch.zeros(size=(1, 112, 32, 32))
    x2 = torch.zeros(size=(1, 320,16, 16))
    outs = model([x0,x1,x2])
...
class Classifier(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, num_classes,
                 num_layers, pyramid_levels=5, onnx_export=False):
...
def demoRegCls():
    ...
    """
        reg.out.shape: torch.Size([1, 11796480, 4])
        cls.out.shape: torch.Size([1, 11796480, 21])
    """
    print('reg.out.shape: {}'.format(reg_out.shape))
    ...
    cls_out = cls([x] * 5)
    print('cls.out.shape: {}'.format(cls_out.shape))
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, compound_coef,num_classes = 21, load_weights=False):
        super(EfficientNet, self).__init__()
        model = EffNet.from_pretrained(f'efficientnet-b{compound_coef}',
                                       load_weights,num_classes=num_classes)
