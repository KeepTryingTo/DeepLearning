...
from utiles.utils import (preprocess, invert_affine, postprocess,
...
def display(imgName,preds, imgs, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        ...
        if imwrite:
            cv2.imwrite(f'runs/img_inferred_d{compound_coef}_{imgName}_{i}.jpg', imgs[i])
...
for imgName in os.listdir(root):
    ...
    display(imgName,out, ori_imgs, imshow=False, imwrite=True)
