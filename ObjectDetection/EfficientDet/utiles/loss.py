...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions,
        ...
        # TODO 得到所有anchor的高宽和中心坐标：[xmin,ymin,xmax,ymax] => [x,y,w,h];
        #  anchor_widths: [49104]
        ...
        for j in range(batch_size):
            #TODO 得到当前batch index所对应的预测类别：[49104,90]
            classification = classifications[j, :, :]
            #TODO 得到当前batch index所对应的预测box：[49104,4]
            regression = regressions[j, :, :]
            #TODO 得到第j张图像中的所有标注的boxes: [max_num_annotas,5]
            bbox_annotation = annotations[j]
            #TODO  针对背景进行过滤: [num_boxes,5]（根据数据打包的情况来看，只需要得到不为-1的值即为当前图像中实际的目标box）
            bbox_annotation = bbox_annotation[bbox_annotation[:, 4] != -1]
            #TODO 对预测得到的classification进行过滤一遍
            ...
            #TODO 如果没有box的话，那么对classification使用focal loss计算损失值
