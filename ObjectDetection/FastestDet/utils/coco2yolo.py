...
def createImagePath(root,save_txt,mode = 'train2014'):
    # TODO 指定要查找的图像文件格式
    image_formats = ['*.jpg', '*.jpeg', '*.png', '*.bmp', '*.gif']
    #todo 保存图像文件路径
    fp = open(os.path.join(save_txt,mode + '.txt'),'w+')
    # 遍历指定的每种图像格式
    for format in image_formats:
        # 使用 glob.glob 找到所有符合格式的图像文件
        for imgName in glob.glob(os.path.join(root, format)):
            fp.write(os.path.join(root,imgName) +' \n')
    fp.close()


if __name__ == "__main__":
    # 示例用法
    coco_json_path = '/home/ff/myProject/KGT/myProjects/myDataset/coco/captions/annotations/instances_val2014.json'  # COCO 的 g注释 JSON 文件路径
    images_dir = '/home/ff/myProject/KGT/myProjects/myDataset/coco/val2014/val2014'  # 包含图像的目录
    output_dir = '/home/ff/myProject/KGT/myProjects/myDataset/coco/yolo/val'  # 输出 YOLO 标签和图像的目录

    # convert_coco_to_yolo(coco_json_path, images_dir, output_dir)
    # createImagePath(
    #     root=r'/home/ff/myProject/KGT/myProjects/myDataset/coco/yolo/train',
    #     save_txt=r'/home/ff/myProject/KGT/myProjects/myDataset/coco/yolo',
    #     mode='val2014'
    # )