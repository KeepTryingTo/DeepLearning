...
parser.add_argument('-c', '--config', default='configs/tiny_COCO.py')
...
print_info('----------------------------------------------------------------------\n'
           '|                       Tiny DSOD Training Program                       |\n'
           '----------------------------------------------------------------------', ['yellow', 'bold'])
#TODO 加载日志logger
logger = set_logger(args.tensorboard)
#TODO 加载配置文件
cfg = Config.fromfile(args.config)
net = Framework(num_class=21)
if args.resume_net:
...
if __name__ == '__main__':
    ...
    for iteration in range(start_iter, max_iter):
        ...
        write_logger({'loc_loss': loss_l.item(),
                      ...
                      'loss': loss.item()}, logger,
                     iteration, status=args.tensorboard)
        loss.backward()
        optimizer.step()
        load_t1 = time.time()

        print_train_log(iteration, cfg.train_cfg.print_epochs,
                        [time.ctime(), epoch, iteration % epoch_size,
                         epoch_size,
