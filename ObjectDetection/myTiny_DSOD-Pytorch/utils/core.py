...
from data import COCODetection,detection_collate,preproc
...
def get_dataloader(cfg, dataset, setname='train_sets'):
    _preproc = preproc(cfg.model.input_size,
                       cfg.model.rgb_means,
                       cfg.model.p)
...
def save_checkpoint(net, cfg, final=True, datasetname='COCO', epoch=10):
    ...
    if final:
        torch.save(net.state_dict(), weights_save_path +
                   'Final_tiny_dsod_{}_size{}.pth'.format(datasetname,
                                                      cfg.model.input_size))
    else:
        torch.save(net.state_dict(), weights_save_path +
                   'tiny_dsod_{}_size{}_epoch{}.pth'.format(datasetname,
