...
class AnchorBox():
    def __init__(self, input_shape, min_size, max_size=None, aspect_ratios=None, flip=True):
        ...
        #min_size,max_size分别对应一个anchor的最小到最大值的范围，在该范围之内根据给定的比例得到anchor高宽
    ...
    def call(self, layer_shape, mask=None):
        ...
        #np.tile类似repeat 在给定的维度上重复多少次;在行上的维度重复1次，在列的维度上重复2 * num_anchors_
        anchor_boxes = np.tile(anchor_boxes, reps = (1, 2 * num_anchors_))
        # 获得先验框的左上角和右下角
...
def get_anchors(input_shape = (300,300), anchors_size = (30, 60, 111, 162, 213, 264, 315), backbone = 'vgg'):
    ...
    for i in range(len(feature_heights)):
        anchor_boxes = AnchorBox(
            input_shape=input_shape, min_size=anchors_size[i],
