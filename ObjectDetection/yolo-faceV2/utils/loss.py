...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        #TODO 根据gt box构建ground truth
        tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets

        # Losses
        for i, pi in enumerate(p):  # layer index, layer predictions
            #TODO 对应图像，anchor以及网格大小
            b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
            # print('b = {} a = {} gj = {} gi = {}'.format(b,a,gj,gi))
            ...
            if n:
                ...
                # TODO 解码预测结果 => [cx,cy,w,h]Regression
                pxy = ps[:, :2].sigmoid() * 2. - 0.5
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
                pbox = torch.cat((pxy, pwh), 1)  # predicted box
                #TODO 计算预测结果和ground truth之间的IOU损失
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
                #TODO 计算Normalized Gaussian Wasserstein Distance损失
                nwd = torch.exp(-torch.pow(Wasserstein(pbox.T,
                                                       tbox[i],
                                                       x1y1x2y2=False), 1 / 2) / self.C)
                auto_iou = iou.mean()
                # TODO 得到IOU损失以及NWD损失 lbox += (1.0 - iou).mean()  # iou loss
                lbox += 0.8 * (1.0 - iou).mean() + 0.2 * (1.0 - nwd).mean()

                # Objectness
                tobj[b, a, gj, gi] = ((1.0 - self.gr) +
                                      self.gr * iou.detach().clamp(0).type(tobj.dtype))  # iou ratio

                # TODO 针对多个类别的损失计算 Classification
                ...
                for id, indexs in dic.items():  # id = batch_name  indexs = target_id
                    #TODO 对于不同的组计算排斥损失值
                    if indexs:
                        lrepgt, lrepbox = repulsion_loss_torch(pbox[indexs],
                                                               tbox[i][indexs],
                                                               deta=deta, #TODO 默认为0.5
                                                               pnms=Rp_nms, #TODO 默认为0.1
                                                               gtnms=Rp_nms)
                        _lrepGT += lrepgt
                        _lrepBox += lrepbox
                        bts += 1
    ...
    def build_targets(self, p, targets):
        ...
        #TODO 获得anchor的数量以及target中对应目标数量
