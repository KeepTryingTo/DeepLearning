...
from retinanet.config import cat_ids,class_name
...
def detect_image(image_path, model_path, class_list):
    ...
    #     labels[value] = key
    labels = cat_ids
    model = resnet50(num_classes=80, pretrained=True)
    ...
    for img_name in os.listdir(image_path):
        ...
        with torch.no_grad():
            ...
            scores, classification, boxes = model(image.cuda().float())
            ...
            for j in range(idxs[0].shape[0]):
                ...
                x2 = int(bbox[2] / scale)
                y2 = int(bbox[3] / scale)
                label_name = class_name[labels[int(classification[idxs[0][j]]) + 1]]
                score = scores[j]
            ...
            # cv2.waitKey(0)
            print('{} inference time is {}'.format(img_name,time.time() - start_time))
            cv2.imwrite('./outputs/' + img_name,image_orig)
