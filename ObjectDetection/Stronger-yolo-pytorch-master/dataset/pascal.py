...
class VOCdataset(BaseDataset):
    ...
    def _parse_annotation(self,itemidx,random_trainsize):
        ...
        img, bboxes = dataAug.img_preprocess2(
            ...
            target_shape = (random_trainsize, random_trainsize),
            correct_box=True)
...
def get_dataset(cfg):
    ...
    trainset = DataLoader(dataset=trainset, batch_size=1,
                          shuffle=True, num_workers=cfg.DATASET.numworker,
                          pin_memory=True)

    subset = [('2007', 'test')]
    valset = VOCdataset(cfg, subset,istrain=False)
    valset = DataLoader(dataset=valset, batch_size=1, shuffle=False,
                        num_workers=cfg.DATASET.numworker, pin_memory=True)
