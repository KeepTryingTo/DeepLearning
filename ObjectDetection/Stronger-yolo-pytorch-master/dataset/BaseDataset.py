...
class BaseDataset(data.Dataset):
    ...
    def preprocess_anchorfree(self, bboxes, labels, outputshapes):
        ...
        for bbox, l in zip(bboxes, labels):
            # TODO (1)获取bbox在原图上的顶点坐标、类别索引、mix up权重、中心坐标、高宽、尺度
            bbox_coor = bbox[:4]
            bbox_class_ind = l
            ...
            bbox_mixw = bbox[4]
            # TODO label smooth
            onehot = np.zeros(self.numcls, dtype=float)
            onehot[bbox_class_ind] = 1.0
            #TODO 均匀分布
            """
                shape: 这里是 self.numcls，指定了数组的大小（即元素数量）。
                fill_value: 这里是 1.0 / self.numcls，指定了数组中每个元素的值。
            """
