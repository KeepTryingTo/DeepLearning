...
class COCOdataset(BaseDataset):
    def __init__(self, cfg,subset,istrain):
        ...
        self.image_dir = "{}/{}2014/{}2014".format(self.dataset_root,subset, subset)
        self.coco = COCO("{}/captions/annotations/instances_{}2014.json".format(self.dataset_root, subset))
...
def get_dataset(cfg):
    ...
    valset = torch.utils.data.DataLoader(dataset=valset,
                                         ...
                                         num_workers=cfg.DATASET.numworker,
                                         pin_memory=True)
    if cfg.debug:
        return valset, valset
    trainset = COCOdataset(cfg, subset='train',istrain=True)
    trainset = torch.utils.data.DataLoader(dataset=trainset, batch_size=cfg.OPTIM.batch_size,
                                           shuffle=True, num_workers=cfg.DATASET.numworker,
                                           pin_memory=True)
