...
class StrongerV3KL(nn.Module):
    def __init__(self,cfg):
        ...
        #TODO 分别针对大的，中等的和小的目标检测头和输出检测结果
        ...
        #TODO 默认使用ASFF，关于ASFF（Adaptive-Selection Feature Fusion）论文我们已经讲过
        # https://mydreamambitious.blog.csdn.net/article/details/144066826
        if cfg.ASFF:
    ...
    def decode_infer(self,output,stride):
        #TODO 针对不同层的输出特征图以及对应步长（大，中，小）
        bz=output.shape[0]
        gridsize=output.shape[-1] #TODO 输出特征图大小

        output=output.permute(0,2,3,1)
        output=output.view(bz,gridsize,gridsize,self.gt_per_grid,5+self.numclass+4)
        #TODO 获得对应的左上角，右下角，坐标变化，置信度以及类别概率
        x1y1,x2y2,variance,conf,prob=torch.split(output,[2,2,4,1,self.numclass],dim=4)
        ...
        #TODO [B, gridsize, gridsize, gt_per_grid, 2]
        xy_grid=torch.stack([shiftx,shifty],dim=4).cuda()
        #TODO 根据网格坐标以及输出结果进行解码操作
        x1y1=(xy_grid+0.5-torch.exp(x1y1))*stride
        x2y2=(xy_grid+0.5+torch.exp(x2y2))*stride
