...
def yololoss(
    ...
    :return:
    """
    GIOUloss_s,conf_loss_s,probloss_s = loss_per_scale(
        conv_sbbox, pred_sbbox,
        label_sbbox, sbboxes,
        strides[0],cfg=cfg
    )
    GIOUloss_m,conf_loss_m,probloss_m = loss_per_scale(
        conv_mbbox, pred_mbbox,
        label_mbbox, mbboxes,
        strides[1],cfg=cfg
    )
    GIOUloss_l,conf_loss_l,probloss_l = loss_per_scale(
        conv_lbbox, pred_lbbox,
        label_lbbox, lbboxes,
        strides[2],cfg=cfg
    )
...
def loss_per_scale(conv, pred, label, bboxes, stride,cfg):
    ...
    #TODO 每个网格单元定义的anchor数量
    numanchor = cfg.gt_per_grid
    #TODO [batch_size, output_size, output_size, anchor_per_scale * (5 + num_class)] =>
