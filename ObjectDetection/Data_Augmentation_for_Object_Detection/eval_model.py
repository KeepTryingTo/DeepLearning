...
from prepare_data import (VocDataset, Normalizer, Resizer,
                          AspectRatioBasedSampler, UnNormalizer,
                          collater)
...
def show_image():
    ...
    dataset_val = VocDataset(config.voc_root_dir, 'val',
                             transform=transforms.Compose([Normalizer(), Resizer()]))
    sampler_val = AspectRatioBasedSampler(dataset_val, batch_size=4, drop_last=False)
    dataloader_val = DataLoader(dataset_val, num_workers=8,
                                collate_fn=collater,
