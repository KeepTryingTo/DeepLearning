...
if __name__=="__main__":
    ...
    root =r'/data1/KTG/myDataset/VOC'
    eval_dataset = VOCDataset(root_dir=os.path.join(root,'VOC2007'),
                              resize_size=[800, 1333],
                               split='test', use_difficult=False,
                              is_train=False, augment=None)
    ...
    model=FCOSDetector(mode="inference",device=device)
    # model=torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
    # print("INFO===>success convert BN to SyncBN")
    # model = torch.nn.DataParallel(model)
    checkpoints = torch.load("./checkpoint/VOC_model_41.pth",map_location=torch.device('cpu'))
    ...
    for img,boxes,classes in eval_loader:
        ...
        num+=1
        # print(num,end='\r')
        print('{} test time is {}'.format(num,time.time() - start_time))
