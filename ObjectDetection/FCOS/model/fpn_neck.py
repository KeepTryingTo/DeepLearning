...
class FPN(nn.Module):
    ...
    def __init__(self,features=256,use_p5=True):
        ...
        if use_p5:
            self.conv_out6 = nn.Conv2d(in_channels=features, out_channels=features,
                                       kernel_size=3, padding=1,
                                       stride=2)
        else:
            self.conv_out6 = nn.Conv2d(in_channels=2048, out_channels=features,
                                       kernel_size=3, padding=1,
                                       stride=2)
        self.conv_out7 = nn.Conv2d(features, features, kernel_size=3,
                                   padding=1, stride=2)
        self.use_p5=use_p5
