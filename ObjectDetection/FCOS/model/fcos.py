...
class DetectHead(nn.Module):
    ...
    def forward(self,inputs):
        ...
        reg_preds   list contains five [batch_size,4,h,w] 
        '''
        #TODO 分别获得分类预测结果，center-ness预测结果以及box回归预测结果
        ...
        if self.config.add_centerness:
            cls_scores = torch.sqrt(
                cls_scores*(cnt_preds.squeeze(dim=-1))
            )#[batch_size,sum(_h*_w)]
        cls_classes=cls_classes+1#[batch_size,sum(_h*_w)]
        #TODO 根据网格坐标中心以及预测的偏移获得[xmin,ymin,xmax,ymax]
        ...
        #TODO 对类别分数进行排序并得到指定的前max_num个分数得到其对应的索引
        topk_ind=torch.topk(cls_scores,max_num,
                            dim=-1,largest=True,
                            sorted=True)[1]#[batch_size,max_num]
        _cls_scores=[]
        _cls_classes=[]
        _boxes=[]
    ...
    def _post_process(self,preds_topk):
        ...
        for batch in range(cls_classes_topk.shape[0]):
            ...
            #TODO
            nms_ind=self.batched_nms(_boxes_b,_cls_scores_b,_cls_classes_b,
                                     self.nms_iou_threshold)

            _cls_scores_post.append(_cls_scores_b[nms_ind])
            _cls_classes_post.append(_cls_classes_b[nms_ind])
            _boxes_post.append(_boxes_b[nms_ind])
        scores,classes,boxes= (torch.stack(_cls_scores_post,dim=0),
    ...
    def batched_nms(self,boxes, scores, idxs, iou_threshold):
        ...
        """
        策略：为了对每个类别独立执行非极大值抑制（NMS），我们将一个偏移量添加到所有框中。
            该偏移量仅依赖于类别索引，并且足够大，以确保不同类别的框不会重叠
        """
        max_coordinate = boxes.max() #TODO 找到box中最大值坐标
    ...
    def _coords2boxes(self,coords,offsets):
        ...
        '''
        #TODO 根据网格坐标以及预测的坐标偏移得到左上角和右下角的坐标
    ...
    def _reshape_cat_out(self,inputs,strides):
        ...
        for pred,stride in zip(inputs,strides):
            ...
            #TODO 根据当前特征图高宽以及下采样步长得到网格中心坐标序列
