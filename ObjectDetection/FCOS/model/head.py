...
class ClsCntRegHead(nn.Module):
        ...
        for i in range(4):
            cls_branch.append(
                nn.Conv2d(in_channel,
                          ...
                          padding=1,bias=True))
            if GN:
                cls_branch.append(nn.GroupNorm(num_groups=32,num_channels=in_channel))
            cls_branch.append(nn.ReLU(True))

            reg_branch.append(nn.Conv2d(in_channels=in_channel,
                                        out_channels=in_channel,
                                        kernel_size=3,padding=1,
                                        bias=True))
            if GN:
                reg_branch.append(nn.GroupNorm(num_groups=32,num_channels=in_channel))
    ...
    def forward(self,inputs):
        ...
        for index,P in enumerate(inputs):
            ...
            #TODO 是把center-ness放在box回归分支还是放在分类分支
