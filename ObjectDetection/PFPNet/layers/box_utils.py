...
def refine_match(threshold, truths, priors,
    ...
    if arm_loc is None:
        ...
        #TODO 使用ARM loc对anchor box进行调整之后，再和gt box计算iou
        decode_arm = decode(arm_loc, priors=priors, variances=variances)
        overlaps = jaccard(truths, decode_arm)
    # (Bipartite Matching)
    # TODO [1,num_objects] best prior for each ground truth
    best_prior_overlap, best_prior_idx = overlaps.max(1, keepdim=True)
    # TODO [1,num_priors] best ground truth for each prior
    best_truth_overlap, best_truth_idx = overlaps.max(0, keepdim=True)
    best_truth_idx.squeeze_(0)
    best_truth_overlap.squeeze_(0)
    best_prior_idx.squeeze_(1)
...
def decode(loc, priors, variances):
    ...
    """
    # print('priors.device: {}'.format(priors.device))
    # print('loc.device: {}'.format(loc.device))
    boxes = torch.cat((
