...
class RefineDetMultiBoxLoss(nn.Module):
    ...
    def __init__(self, num_classes, overlap_thresh, prior_for_matching,
                 ...
                 device = 'cpu', theta=0.01, use_ARM=False):
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num):
            ...
            #TODO
            if self.use_ARM:
                refine_match(threshold=self.threshold,
                             truths=truths, priors=defaults,
                             variances=self.variance, labels=labels,
                             loc_t=loc_t, conf_t=conf_t, idx=idx,
                             arm_loc=arm_loc_data[idx].data)
            else:
                refine_match(threshold=self.threshold, truths=truths,
