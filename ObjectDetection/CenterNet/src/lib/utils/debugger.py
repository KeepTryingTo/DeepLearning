...
class Debugger(object):
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    #TODO 保存绘制的图像
    save = r'/home/ff/myProject/KGT/myProjects/myProjects/CenterNet-master/outputs'
    # print('save img {}'.format(img_id))
    cv2.imwrite(os.path.join(save,img_id + '.png'),self.imgs[img_id])
    ...
    if not self.ipynb:
      ...
      for i, (k, v) in enumerate(self.imgs.items()):
        ...
        if len(v.shape) == 3:
          ...
          # self.plt.savefig("../../../outputs/result.png")
        else:
          # self.plt.savefig("../../../outputs/result.png")
          self.plt.imshow(v)
      #TODO 服务器上暂时不支持显示操作
