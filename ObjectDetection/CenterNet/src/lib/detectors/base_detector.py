...
class BaseDetector(object):
  def __init__(self, opt):
    ...
    #TODO 我们这里默认构建resnet18
    print('Creating model and head_conv: {} ...'.format(opt.head_conv))
    self.model = create_model(arch=opt.arch,
                              heads=opt.heads,
  ...
  def run(self, image_or_path_or_tensor, meta=None):
    #TODO 这里默认传进来的image_or_path_or_tensor为图像相对路径
    ...
    if self.opt.debug >= 1:
      print('debug {} show detection img...'.format(self.opt.debug))
