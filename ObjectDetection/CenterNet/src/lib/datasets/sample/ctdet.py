...
class CTDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    for k in range(num_objs):
      ...
      if h > 0 and w > 0:
        ...
        if self.opt.dense_wh:
          draw_dense_reg(
            regmap=dense_wh, heatmap=hm.max(axis=0),
            center=ct_int, value=wh[k], radius=radius
          )
    ...
    if self.opt.dense_wh:
      ...
      del ret['wh']
    elif self.opt.cat_spec_wh: #TODO 默认为false
