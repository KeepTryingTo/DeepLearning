...
class ContextTexture (nn.Module):
    ...
    def __init__(self, **channels):
        ...
        self.up_conv = nn.Conv2d(channels['up'],
                                 channels['main'],
                                 kernel_size=1)
        self.main_conv = nn.Conv2d(channels['main'],
                                   channels['main'],
                                   kernel_size=1)
    ...
    def forward(self,up,main):
        ...
        _,_,H,W = main.size()
        res = F.interpolate(input=up, scale_factor=2, mode='bilinear')
...
class SFD(nn.Module):
    def __init__(self, block, num_blocks,
        ...
        # TODO 根据输出的每一层所对应的脸部anchor box尺寸，头部anchor box尺寸以及身体anchor box尺寸
        # todo 可以看到anchor box的尺寸是依次增加的，作为上下文信息
        ...
        #TODO LFPN(Low-Feature Pyramid Network)
        ...
        self.smooth_c3 = nn.Conv2d(256,256,
                                   kernel_size=3,padding=1)
        self.smooth_c4 = nn.Conv2d(512,512,
                                   kernel_size=3,padding=1)
        self.smooth_c5 = nn.Conv2d(1024,1024,
                                   kernel_size=3,padding=1)
        ...
        #TODO 对于面部的预测层，预测置信度部分，输出的通道数为4, = Cpl + Cnl分别对
        # 应于前景和背景标签的最大值
        loc = []
        conf = []
        for i in range(6):
            loc.append(nn.Conv2d(self.SSHchannels[i],out_channels=4,
                                 kernel_size=3,stride=1,padding=1))
            conf.append(nn.Conv2d(self.SSHchannels[i],out_channels=4,
                                  kernel_size=3,stride=1,padding=1))

        self.face_loc = nn.ModuleList(loc)
        self.face_conf = nn.ModuleList(conf)

        #TODO 对于head和body的置信度预测输出通道数为2，表示背景-0和背景-1
        head_loc = []
        head_conf = []
        for i in range(5):
            head_loc.append(nn.Conv2d(self.SSHchannels[i+1],out_channels=4,
                                      kernel_size=3,stride=1,padding=1))
            head_conf.append(nn.Conv2d(self.SSHchannels[i+1],out_channels=2,
                                       kernel_size=3,stride=1,padding=1))
        ...
        body_conf = []
        for i in range(4):
            body_loc.append(nn.Conv2d(self.SSHchannels[i+2],out_channels=4,
                                    kernel_size=3,stride=1,padding=1))
            body_conf.append(nn.Conv2d(self.SSHchannels[i+2],out_channels=2,
                                    kernel_size=3,stride=1,padding=1))
        ...
        if phase == 'test':
            ...
            self.detect = Detect(num_classes=num_classes, bkg_label=0,
                                 top_k=750, conf_thresh=0.05,
                                 nms_thresh=0.3)
    ...
    def forward(self, x):
        ...
        for idx, f_layer in enumerate(sources):
            ...
            prior_boxs.append(
                self.priorbox.forward(
                    idx,
                    f_layer.shape[3], #TODO 根据当前特征图的大小生成anchor
                    f_layer.shape[2]
                )
            )
            if idx > 0:
                prior_head_boxes.append(
                    self.priorbox_head.forward(
                        idx-1,
                        f_layer.shape[3],
                        f_layer.shape[2]
                    )
        ...
        self.priors = Variable(torch.cat([p for p in prior_boxs],dim = 0))
        self.priors_head = Variable(torch.cat([p for p in prior_head_boxes],dim = 0))
        ...
        for idx, (x, l, c) in enumerate(zip(sources, self.face_loc, self.face_conf)):
            if idx==0:
                ...
                conf.append(torch.cat([neg_conf,max_conf],
                                      dim = 1).permute(0,2,3,1).contiguous())
...
def build_sfd(phase, size=640, num_classes=2):
    ...
    if size != 640:
        ...
        return
    #TODO 这里以ResNet50作为backbone
    return SFD(block=Bottleneck, num_blocks = [3,4,6,3],
               phase=phase , num_classes=num_classes, size=size)

def demo():
    x = torch.zeros(size=(1,3,640,640))
    model = build_sfd(phase='train')
    outs = model(x)
    """
        out.shape: torch.Size([1, 34125, 4])
        out.shape: torch.Size([1, 34125, 2])
        out.shape: torch.Size([34125, 4])
        out.shape: torch.Size([1, 8525, 4])
        out.shape: torch.Size([1, 8525, 2])
        out.shape: torch.Size([8525, 4])
    """
    for out in outs:
