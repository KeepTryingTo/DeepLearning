...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45):
    ...
    for i, image_pred in enumerate(prediction):
        # TODO Filter out confidence scores below threshold获得预测的最大类别概率
        class_pred = torch.max(image_pred[:, 5:5 + num_classes], dim = 1)
        ...
        if not image_pred.size(0):
            continue
        # TODO Get score and class with highest confidence
        class_conf, class_pred = torch.max(
            image_pred[:, 5:5 + num_classes],
            dim = 1,  keepdim=True
        )
        
        # TODO Detections ordered as (x1, y1, x2, y2, obj_conf, class_conf, class_pred)
        ...
        if gaussian:
            #TODO  Get uncertainties
            sigma = image_pred[:, -4:]
            # todo Detections ordered as # (x1, y1, x2, y2, obj_conf, class_conf, class_pred,
            # todo sigma_x, sigma_y, sigma_w, sigma_h)
            detections = torch.cat((detections, sigma), 1)

        # todo Iterate through all predicted classes
        unique_labels = detections[:, 6].cpu().unique()
        if prediction.is_cuda:
            unique_labels = unique_labels.cuda()
        #TODO 对于每一个类别都进行NMS算法
        for c in unique_labels:
            # TODO 获得当前类别对应的所有检测结果 Get the detections with the particular class
