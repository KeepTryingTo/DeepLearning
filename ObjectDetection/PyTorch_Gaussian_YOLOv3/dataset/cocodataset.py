...
class COCODataset(Dataset):
    ...
    COCO dataset class.
    """
    def __init__(self, model_type, data_dir='COCO', json_file='instances_train2014.json',
                 name='train2014', img_size=416,
        ...
        self.coco = COCO(os.path.join(self.data_dir,'captions/annotations/'+self.json_file))
    ...
    def __getitem__(self, index):
        ...
        img_file = os.path.join(self.data_dir, self.name,self.name,
                            'COCO_{}_'.format(self.name) + '{:012}'.format(id_) + '.jpg')
        img = cv2.imread(img_file)

        if self.json_file == 'instances_val5k.json' and img is None:
            img_file = os.path.join(self.data_dir, 'train2014',
                                    '{:012}'.format(id_) + '.jpg')
            img = cv2.imread(img_file)
