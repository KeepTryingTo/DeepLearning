...
def refine_match(threshold, truths, priors,
                 variances, labels, loc_t,
    ...
    if arm_loc is None:
        ...
        #TODO 将经过refine 模块调整之后的anchor box再和gt box计算IOU
    ...
    if arm_loc is None:
        ...
        conf = labels[best_truth_idx] + 1     # Shape: [num_priors]
        #TODO center size =>[xmin,ymin,xmax,ymax] => [cx,cy,w,h]
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    while idx.numel() > 0:
        ...
        if idx.size(0) == 1:
            break
        idx = idx[:-1]  #TODO  remove kept element from view
        ...
        w = xx2 - xx1
        h = yy2 - yy1
        # TODO check sizes of xx1 and xx2.. after each iteration
        w = torch.clamp(w, min=0.0)
        h = torch.clamp(h, min=0.0)
        inter = w*h
        # TODO IoU = i / (area(a) + area(b) - i)
