...
class RefineDet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, ARM, ODM, TCB, num_classes):
        ...
        if phase == 'test':
            ...
            self.detect = Detect_RefineDet(num_classes=num_classes,
                                           size=self.size, bkg_label=0,
                                           top_k=1000, conf_thresh=0.01,
                                           nms_thresh=0.15,
    ...
    def forward(self, x):
        ...
        # TODO apply ARM and ODM to source layers
        ...
        for (x, l, c) in zip(tcb_source, self.odm_loc, self.odm_conf):
            loc_odm = l(x).permute(0, 2, 3, 1).contiguous()
            cls_odm = c(x).permute(0, 2, 3, 1).contiguous()
            odm_loc.append(loc_odm)
            odm_conf.append(cls_odm)

            """
                loc dom.size: torch.Size([1, 40, 40, 12])
                loc dom.size: torch.Size([1, 20, 20, 12])
                loc dom.size: torch.Size([1, 10, 10, 12])
                loc dom.size: torch.Size([1, 5, 5, 12])
            """
            # print('loc dom.size: {}'.format(loc_odm.size()))
...
def add_tcb(cfg):
    ...
    for k, v in enumerate(cfg):
        ...
        if k != len(cfg) - 1:
            feature_upsample_layers += [nn.ConvTranspose2d(in_channels=256, out_channels=256,
                                                           kernel_size=2, stride=2)]
    return (feature_scale_layers,
            feature_upsample_layers,
            feature_pred_layers)
...
def build_refinedet(phase, size=320, num_classes=21):
    ...
    return RefineDet(phase, size, base_, extras_,
                     ARM_, ODM_, TCB_, num_classes)


def demo():
    x = torch.randn(1, 3, 320, 320)
    model = build_refinedet(phase='train',
                            size=320,
                            num_classes=21)
    model.eval()
    outs = model(x)
