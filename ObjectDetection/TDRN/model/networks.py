...
def conv_offset2d(input,
    ...
    return f.forward(input, offset, weight)
...
class ConvOffset2dFunction(Function):
    ...
    def forward(self, input, offset, weight):
        ...
        if not input.is_cuda:
            ...
            output = deform_conv.deform_conv2d(
                input=input, offset=offset, weight=weight,
                stride=(self.stride[1], self.stride[0]),
                padding=(self.padding[1], self.padding[0]),
                dilation=(self.dilation[1],self.dilation[0]))
        return output
