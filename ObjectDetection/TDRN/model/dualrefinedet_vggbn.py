...
from layers import *
from model.networks import vgg, vgg_base, ConvOffset2d

class RefineSSD(nn.Module):

    def __init__(self,size, num_classes=21, phase='train',
                 c7_channel=1024, def_groups=1, bn=True,
                 multihead=False, return_feature=False,
                 device='cuda'):
        ...
        # TODO 以VGG作为backbone SSD network
        self.backbone = nn.ModuleList(vgg(vgg_base['320'], 3,
                                          batch_norm=self.bn,
                                          pool5_ds=True,
    ...
    def forward(self, x):
        ...
        # TODO TCB get transformed layers
        ...
        obm_sources.reverse()
        if self.multihead:
            for (ob, l, l2, c, c2, f, f2) in zip(obm_sources, self.odm_loc,
                                                 self.odm_loc_2, self.odm_conf,
                                                 self.odm_conf_2, arm_offset_list,
                                                 arm_offset2_list):
            ...
            for (ob, l, c, f) in zip(obm_sources, self.odm_loc,
                                     self.odm_conf, arm_offset_list):
...
def build_net(phase, size=320, num_classes=21,
              ...
              bn=True, multihead=False,
              return_feature=False):
    if size not in [320, 512]:
        print("Error: Sorry only SSD320 and SSD512 is supported currently!")
        return

    return RefineSSD(size, num_classes=num_classes,
                     phase=phase, c7_channel=c7_channel,
                     def_groups=def_groups, bn=bn,
                     multihead=multihead, return_feature=return_feature)


def demo():
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    x = torch.zeros(size=(2,3,320,320)).to(device)
    model = build_net(phase='train',size=320).to(device)
    outs = model(x)
    for out in outs:
        if out is not None:
