...
from data import (AnnotationTransform, BaseTransform, VOCDetection,
                  detection_collate,
                  seq_detection_collate, mb_cfg, dataset_training_cfg,)
...
parser.add_argument('--resume', default=r'weights/tdrn320_VOC0712_370.pth', type=str, help='Resume from checkpoint') #'./weights/tssd300_VID2017_b8s8_RSkipTBLstm_baseAugmDrop2Clip5_FixVggExtraPreLocConf/ssd300_seqVID2017_20000.pth'
...
parser.add_argument('--model_name', default='tdrn', type=str, help='which model selected')
...
#TODO 获得要训练的数据集名称以及对应的类别数，以及打印信息
...
#TODO 默认backbone为RefineDet_ResNet101
    ...
    if 'MobNet' in args.backbone:
        ...
        ssd_net = build_net('train', size=ssd_dim,
                            ...
                            def_groups=args.deform, bn=args.bn,
                            multihead=args.multihead)
        ...
        ssd_net = build_net('train', size=ssd_dim,
                            ...
                            use_refine=args.refine,
                            c7_channel=args.c7_channel,
                            bn=args.bn, multihead=args.multihead)
...
if not args.resume:
    ...
    net_init(ssd_net, args.backbone, logging,
             refine=args.refine, deform=args.deform,
             multihead=args.multihead)

#TODO 默认使用SSD中的数据增强方式
...
optimizer = optim.SGD(net.parameters(),
                      ...
                      weight_decay=args.weight_decay)
# TODO 默认会采用第一个criterion
if 'RefineDet' in args.backbone and args.refine:
    use_refine = True
    arm_criterion = RefineMultiBoxLoss(num_classes=2, overlap_thresh=0.5,
                                       prior_for_matching=True, bkg_label=0,
                                       neg_mining=True, neg_pos=3,
                                       neg_overlap=0.5, encode_target=False,
                                       device=device, only_loc=True)
    criterion = RefineMultiBoxLoss(num_classes, overlap_thresh=0.5,
                                   prior_for_matching=True, bkg_label=0,
                                   neg_mining=True, neg_pos=3,
                                   neg_overlap=0.5, encode_target=False,
                                   device=device)
...
def train():
    ...
    max_iter = max_epoch * epoch_size * 10
        ...
        if (not batch_iterator) or (iteration % epoch_size == 0):
            ...
            if epoch % args.save_interval == 0:
                ...
                torch.save(ssd_net.state_dict(),
                           os.path.join(args.save_folder, args.model_name + str(
