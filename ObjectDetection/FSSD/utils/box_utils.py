...
def match(threshold, truths,
    ...
    """
    # jaccard index TODO 计算交并比 [A,B]
    overlaps = jaccard(
        truths,
        point_form(priors)
    )
    # (Bipartite Matching)
    # [1,num_objects] best prior for each ground truth TODO 在B(anchor)的维度上找到和A最佳匹配的anchor坐标框
    ...
    #TODO 同时将anchor和gt box匹配最佳的，并且在best_truth_idx中的都设置为j，表示当前第j个gt box和第best_prior_idx[j]个anchor匹配
    ...
    #TODO 编码gt box和anchor之间的偏移值，作为网络模型学习的值
...
def encode(matched, priors, variances):
    ...
    """
    #TODO 以下的编码过程完全是采用了SSD算法的编码方式
    # dist b/t match center and prior's center
    # TODO 首先计算gt box的中心坐标，和anchor最佳匹配的框中心坐标之间计算偏移
    g_cxcy = (matched[:, :2] + matched[:, 2:])/2 - priors[:, :2]
    # encode variance TODO 中心偏移 / 高宽的变化值
    g_cxcy /= (variances[0] * priors[:, 2:])
    # match wh / prior wh TODO gt box的高宽和anchor的高宽之间比值
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    keep = torch.zeros(size=(scores.size(0),),dtype=torch.long)
    if boxes.numel() == 0:
        return keep
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
