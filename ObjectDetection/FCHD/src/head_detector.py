...
class Head_Detector(nn.Module):
    ...
    def get_optimizer(self):
        ...
        if opt.use_adam:
            self.optimizer = t.optim.Adam(params,lr=lr)
        else:
            self.optimizer = t.optim.SGD(params, momentum=0.9,lr=lr)
    ...
    def _suppress(self, raw_cls_bbox, raw_prob, nms_thresh, score_thresh):
        ...
        # print('type(bbox): {}'.format(type(bbox)))
        keep = nms(boxes=torch.tensor(bbox),
