...
def data_line_parser(line, dataset):
    if b":" not in line:
        img_path, _ = line.split(b";")
        if dataset == 'hollywood':            
            src_path = os.path.join(opt.hollywood_dataset_root_path,'Images',img_path.replace(b'"',b'').decode('utf-8'))
        elif dataset == 'brainwash':
            src_path = os.path.join(opt.brainwash_dataset_root_path, img_path.replace(b'"',b'').decode('utf-8'))
        num_coordinates = 0
        coordinates = []
        return {'img_path':src_path, 'number': num_coordinates,'coordinates':coordinates}
    else:
        img_path, bbox_coordinates_raw = line.split(b":")
        if dataset == 'hollywood':
            src_path = os.path.join(opt.hollywood_dataset_root_path,'Images',img_path.replace(b'"',''))
        elif dataset == 'brainwash':
            src_path = os.path.join(opt.brainwash_dataset_root_path, img_path.replace(b'"', b'').decode('utf-8'))
        bbox_coordinates_raw = bbox_coordinates_raw.replace(b"(",b"")
        bbox_coordinates_raw = bbox_coordinates_raw.replace(b"),",b",")
        bbox_coordinates_raw = bbox_coordinates_raw.replace(b").",b"")
        bbox_coordinates_raw = bbox_coordinates_raw.replace(b");",b"")
        bbox_coordinates_str = bbox_coordinates_raw.split(b", ")
        coordinates_list = [float(i) for i in bbox_coordinates_str]
        num_coordinates = len(coordinates_list)/4
        coordinates = np.zeros(shape=(int(num_coordinates),4),dtype=float)
        entry_idx = 0
...
def check_loaded_data(d):
    ...
    #TODO 读取图像 print bboxs.shape
    image = cv2.imread(path)
    image_np = np.copy(np.asarray(image, dtype=np.uint8))
    #TODO 获取当前图像对应的标注坐标以及绘制到图像，目的在于查看是否加载数据集有问题
