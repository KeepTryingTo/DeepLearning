...
class Head_Detector_Trainer(nn.Module):
    ...
    def forward(self, imgs, bboxs, scale):
        ...
        _, _, H, W = imgs.size()
        img_size = (H, W)
        #TODO 经过backbone特征提取器
        features = self.head_detector.extractor(imgs)
        #TODO 获得预测的没有经过裁剪的rpn_locs以及rpn_scores(未经过NMS算法)，和裁剪的rois以及rois_score（经过了NMS算法），anchor box
        ...
        # ------------------ RPN losses -------------------#
        #TODO 计算gt box和anchor box之间的IOU以及对gt box进行编码
