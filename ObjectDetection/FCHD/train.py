...
def eval(dataloader, head_detector):
    ...
    for _, (img, bbox_, scale) in enumerate(dataloader):
        ...
        if pred_bboxes_.shape[0] == 0 or np.ndim(pred_bboxes_) <= 1:
            test_img_num += 1
            continue
        else:
            # print('pred boxes.shape: {}'.format(np.shape(pred_bboxes_)))
            # print('gt boxes.shape: {}'.format(np.shape(gt_bboxs)))
...
def train():    
    # TODO 默认采用brainwash数据集Get the dataset
    for phase in phases:
        #TODO 分别构建训练集，验证集以及测试集
        if phase == 'train':
    ...
    #TODO 打包数据集 Load the train dataset
    train_dataset = Dataset(train_data_list)
    test_dataset = Dataset(val_data_list)
    print ("Load data.")
    train_dataloader = data_.DataLoader(train_dataset, batch_size=1,
                                        shuffle=True, num_workers=8)
    test_dataloader = data_.DataLoader(test_dataset, batch_size=1,
                                       shuffle=True, num_workers=8)
    # TODO 加载模型 Initialize the head detector.
