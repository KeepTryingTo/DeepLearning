...
class CausalBoW(nn.Module):
    """TODO
        有因果关系的词汇。将前面的元素平均，看起来很像transformer中的CausalAttention模块，没有任何明显的原因
    """
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, x):
        B, T, C = x.size() # TODO batch size, sequence length, n_embd

        # TODO do the weighted average of all preceeding token features
        att = torch.zeros((B, T, T), device=x.device)
        att = att.masked_fill(self.bias[:,:T,:T] == 0, float('-inf'))
        att = F.softmax(att, dim=-1)
        y = att @ x # TODO (B, T, T) x (B, T, C) -> (B, T, C)
