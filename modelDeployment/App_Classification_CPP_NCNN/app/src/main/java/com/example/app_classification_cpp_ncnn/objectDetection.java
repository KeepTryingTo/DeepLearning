...
public class objectDetection extends AppCompatActivity implements View.OnClickListener,Runnable {
    ...
    public native float[] ObjectDetect(Bitmap bitmap,float[] NO_MEAN_RGB,
...
//                startActivity(intentVideo);
    ...
    public void run() {
        ...
        //对图像缩放到指定的大小
        bitmap = classifyUtils.preprocessImage(mBitmap,img_size,img_size,true);

        //predictions: [x,y,w,h,conf,class,x,y,w,h,conf,class,...]
        float [] predictions = ObjectDetect(
                bitmap,NO_MEAN_RGB,NO_STD_RGB,img_size,false
        );
