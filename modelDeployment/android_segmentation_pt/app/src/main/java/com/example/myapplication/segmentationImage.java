...
public class segmentationImage extends AppCompatActivity implements Runnable, View.OnClickListener {
    ...
    private String modelName = "best_deeplabv3plus_mobilenet_voc_os16";
    private Uri uri = null;
    private int img_size = 513;
    private static String[] model_name_list = {"best_deeplabv3plus_mobilenet_voc_os16"};
    private float[][][][]out;
    private float[][][][]aux;
        ...
        select_model.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                if(modelName.length() > 0){
                    ...
                    //读取类别文件
                    //虽然raw文件夹中的文件默认是可以读取的，但如果你的代码尝试以某种方式
                    //（如通过文件路径直接访问）来读取这些文件，可能会遇到权限问题。
                    try {
                        if(modelName.equals("best_deeplabv3plus_mobilenet_voc_os16")){
                            img_size = 513;
                            platte = utls.readClassesFile(context, "platte.txt");
                            model = utls.loadModel(context,"best_deeplabv3plus_mobilenet_voc_os16.pt");
                        }
...
//        final IValue[] outputTensor = model.forward(IValue.from(inputData)).toTuple();
        final Tensor outputTensor = model.forward(IValue.from(inputData)).toTensor();

        long[]sizes = outputTensor.shape();
        ...
        float[]flatData = outputTensor.getDataAsFloatArray();
        int index = 0;
        for(int n = 0 ; n < N; n++){
            for(int c = 0 ; c < C; c++){
                for(int i = 0 ; i < H; i++){
                    for(int j = 0 ; j < W; j++){
                        index = n * C * H * W + c * H * W + i * W +j;
