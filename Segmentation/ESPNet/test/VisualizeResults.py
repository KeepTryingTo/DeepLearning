...
def evaluateModel(args, model, up, image_list):
    ...
    for i, imgName in enumerate(image_list):
        ...
        if args.colored:
            classMap_numpy_color = np.zeros(shape = (img.shape[1], img.shape[2], img.shape[0]), dtype=np.uint8)
...
def main(args):
    ...
    if args.modelType == 2:
        modelA = Net.ESPNet_Encoder(classes, p=p, q=q)  # Net.Mobile_SegNetDilatedIA_C_stage1(20)
